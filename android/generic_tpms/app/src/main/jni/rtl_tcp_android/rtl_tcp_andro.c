...
static void sighandler(int signum)
{
	...
	if (signum != 0) {
		...
		announce_exceptioncode(com_fleetcents_generic_tpms_core_RtlTcp_EXIT_SIGNAL_CAUGHT);
	}
...
void rtltcp_main(int usbfd, const char * uspfs_path_input, int argc, char **argv)
{
	...
	if (usbfd != -1 && (!(fcntl(usbfd, F_GETFL) != -1 || errno != EBADF))) {
		...
		announce_exceptioncode(com_fleetcents_generic_tpms_core_RtlTcp_EXIT_INVALID_FD);
		pthread_mutex_lock(&running_mutex);
		is_running = 0;
		pthread_mutex_unlock(&running_mutex);
		return;
	...
	while ((opt = getopt(argc, argv, "a:p:f:g:s:b:n:d:P:")) != -1) {
		...
		default:
			...
			announce_exceptioncode(com_fleetcents_generic_tpms_core_RtlTcp_EXIT_WRONG_ARGS);
			pthread_mutex_lock(&running_mutex);
			is_running = 0;
			pthread_mutex_unlock(&running_mutex);
			return;
	...
	if (usbfd == -1) {
		...
		if (dev_index < 0) {
			...
			announce_exceptioncode( com_fleetcents_generic_tpms_core_RtlTcp_EXIT_NO_DEVICES );
			pthread_mutex_lock(&running_mutex);
			is_running = 0;
			pthread_mutex_unlock(&running_mutex);
			return;
	...
	if (NULL == dev) {
		...
		announce_exceptioncode( com_fleetcents_generic_tpms_core_RtlTcp_EXIT_FAILED_TO_OPEN_DEVICE );
		pthread_mutex_lock(&running_mutex);
		is_running = 0;
		pthread_mutex_unlock(&running_mutex);
		return;
	...
	if (r < 0) {
		...
		announce_exceptioncode( com_fleetcents_generic_tpms_core_RtlTcp_EXIT_NOT_ENOUGH_POWER );
		pthread_mutex_lock(&running_mutex);
		is_running = 0;
		pthread_mutex_unlock(&running_mutex);
		return;
	...
	closesocket(s);
	announce_exceptioncode(com_fleetcents_generic_tpms_core_RtlTcp_EXIT_OK);
	pthread_mutex_lock(&running_mutex);
	is_running = 0;
	pthread_mutex_unlock(&running_mutex);
	aprintf("bye!");
