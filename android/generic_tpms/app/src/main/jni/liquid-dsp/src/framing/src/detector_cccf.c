                                   ...
                                   float           _dphi_max)
{
    // validate input
    if (_n == 0) {
        LOGE("error: detector_cccf_create(), sequence length cannot be zero\n");
        exit(1);
    } else if (_threshold <= 0.0f) {
        LOGE("error: detector_cccf_create(), threshold must be greater than zero (0.6 recommended)\n");
        exit(1);
    }
...
{
    LOGI("detector_cccf:\n");
    LOGI("    sequence length     :   %-u\n", _q->n);
    LOGI("    threshold           :   %8.4f\n", _q->threshold);
    LOGI("    maximum carrier     :   %8.4f rad/sample\n", _q->dphi_max);
    LOGI("    num. correlators    :   %u\n", _q->m);
}
                            ...
                            float *       _gamma_hat)
{
    ...
    if (_q->state == DETECTOR_STATE_SEEK) {
        ...
        if (rxy_abs > _q->threshold) {
            ...
            LOGI("threshold exceeded:      rxy = %8.4f\n", rxy_abs);
#endif
            _q->idetect = _q->imax;
...
#if DEBUG_DETECTOR_PRINT
            LOGI("maximum not yet reached: rxy = %8.4f\n", rxy_abs);
#endif
        ...
        LOGE("error: detector_cccf_correlate(), unknown/unsupported internal state\n");
        exit(1);
    }
...
#if DEBUG_DETECTOR_PRINT
        LOGI("%6.4f (%6.4f) ", _q->rxy[k], _q->dphi[k]);
#endif
                               ...
                               const char *  _filename)
{
    ...
    if (!fid) {
        LOGE("error: detector_cccf_debug_print(), could not open '%s' for writing\n", _filename);
        return;
    }
    ...
    fclose(fid);
    LOGI("detector_cccf/debug: results written to '%s'\n", _filename);
#else
    LOGE("detector_cccf_debug_print(): compile-time debugging disabled\n");
#endif
}
