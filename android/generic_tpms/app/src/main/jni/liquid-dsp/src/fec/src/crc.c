...
crc_scheme liquid_getopt_str2crc(const char * _str)
{
    ...
    LOGE("warning: liquid_getopt_str2crc(), unknown/unsupported crc scheme : %s\n", _str);
...
unsigned int crc_get_length(crc_scheme _scheme)
{
    ...
    default:
        LOGE("error: crc_get_length(), unknown/unsupported scheme: %d\n", _scheme);
        exit(1);
    }
                              ...
                              unsigned int _n)
{
    ...
    case LIQUID_CRC_UNKNOWN:
        LOGE("error: crc_generate_key(), cannot generate key with CRC type \"UNKNOWN\"\n");
        exit(-1);
    case LIQUID_CRC_NONE:      return 0;
    ...
    default:
        LOGE("error: crc_generate_key(), unknown/unsupported scheme: %d\n", _scheme);
        exit(1);
    }
                         ...
                         unsigned int _key)
{
    if (_scheme == LIQUID_CRC_UNKNOWN) {
        LOGE("error: crc_validate_message(), cannot validate with CRC type \"UNKNOWN\"\n");
        exit(-1);
