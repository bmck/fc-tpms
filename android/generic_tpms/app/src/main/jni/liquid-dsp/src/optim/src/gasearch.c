                                  ...
                                  float _mutation_rate)
{
    ...
    if (_population_size > LIQUID_GA_SEARCH_MAX_POPULATION_SIZE) {
        LOGE("error: gasearch_create(), population size exceeds maximum\n");
        exit(1);
    } else if (_mutation_rate < 0.0f || _mutation_rate > 1.0f) {
        LOGE("error: gasearch_create(), mutation rate must be in [0,1]\n");
        exit(1);
    }
    ...
    //LOGI("num_parameters: %d\n", ga->num_parameters);
    //LOGI("population_size: %d\n", ga->population_size);
...
void gasearch_print(gasearch _g)
{
    LOGI("ga search :\n");
    LOGI("    num traits      :   %u\n", _g->num_parameters);
    LOGI("    bits/chromosome :   %u\n", _g->bits_per_chromosome);
    LOGI("    population size :   %u\n", _g->population_size);
    LOGI("    selection size  :   %u\n", _g->selection_size);
    LOGI("    mutation rate   :   %12.8f\n", _g->mutation_rate);
    LOGI("population:\n");
    unsigned int i;
    for (i=0; i<_g->population_size; i++) {
        LOGI("%4u: [%8.4f] ", i, _g->utility[i]);
                                  ...
                                  unsigned int _selection_size)
{
    ...
    if (_population_size < 2) {
        LOGE("error: gasearch_set_population_size(), population must be at least 2\n");
        exit(1);
    } else if (_selection_size == 0) {
        LOGE("error: gasearch_set_population_size(), selection size must be greater than zero\n");
        exit(1);
    } else if (_selection_size >= _population_size) {
        LOGE("error: gasearch_set_population_size(), selection size must be less than population\n");
        exit(1);
    }
                                ...
                                float _mutation_rate)
{
    if (_mutation_rate < 0.0f || _mutation_rate > 1.0f) {
        LOGE("error: gasearch_set_mutation_rate(), mutation rate must be in [0,1]\n");
        exit(1);
    }
...
void gasearch_evolve(gasearch _g)
{
                                ...
                                _g->minimize) )
    {
        ...
        LOGI("  utility: %0.2E", _g->utility_opt);
...
void gasearch_crossover(gasearch _g)
{
    ...
    for (i=_g->selection_size; i<_g->population_size; i++) {
        ...
        //LOGI("  gasearch_crossover, p1: %d, p2: %d, c: %d\n", p1, p2, c);
