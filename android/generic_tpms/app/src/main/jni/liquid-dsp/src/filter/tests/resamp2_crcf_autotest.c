...
void autotest_resamp2_analysis()
{
    ...
    LOGI("close all\n");
    for (i=0; i<2*n; i++)
        LOGI("x(%3u) = %12.4e + j*%12.4e;\n", i+1, crealf(x[i]), cimagf(x[i]));

    for (i=0; i<n; i++) {
        LOGI("y0(%3u) = %12.4e + j*%12.4e;\n", i+1, crealf(y0[i]), cimagf(y0[i]));
        LOGI("y1(%3u) = %12.4e + j*%12.4e;\n", i+1, crealf(y1[i]), cimagf(y1[i]));
    }

    // save expected values
    for (i=0; i<n; i++) {
        LOGI("z0(%3u) = %12.4e + j*%12.4e;\n", i+1, cosf(i*2*f0), sinf(i*2*f0));
        LOGI("z1(%3u) = %12.4e + j*%12.4e;\n", i+1, cosf(i*2*f1), sinf(i*2*f1));
    }

    LOGI("m = %u;\n", m);
    LOGI("figure;\n");
    LOGI("t = 0:(length(y0)-1);\n");
    //LOGI("plot(t,real(z0),t-m+0.5,real(y0));\n");
    LOGI("plot(t,real(z1),t-m+0.5,real(y1));\n");
    fclose(fid);
...
void autotest_resamp2_synthesis()
{
    ...
    LOGI("figure;\n");
    LOGI("t = 0:(length(y)-1);\n");
    LOGI("plot(t,real(z),t-2*m,real(y));\n");
    fclose(fid);
    LOGI("results written to '%s'\n","resamp2_test.m");
#endif
}
