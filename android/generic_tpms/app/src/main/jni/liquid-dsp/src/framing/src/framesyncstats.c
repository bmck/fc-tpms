...
void framesyncstats_print(framesyncstats_s * _stats)
{
    ...
    if (_stats->mod_scheme >= LIQUID_MODEM_NUM_SCHEMES) {
        LOGE("error: framesyncstats_print(), invalid modulation scheme\n");
        exit(1);
     } else if (_stats->check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE("error: framesyncstats_print(), invalid CRC scheme\n");
        exit(1);
     } else if (_stats->fec0 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("error: framesyncstats_print(), invalid FEC scheme (inner)\n");
        exit(1);
     } else if (_stats->fec1 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("error: framesyncstats_print(), invalid FEC scheme (outer)\n");
        exit(1);
    }

    LOGI("    EVM                 :   %12.8f dB\n", _stats->evm);
    LOGI("    rssi                :   %12.8f dB\n", _stats->rssi);
    LOGI("    carrier offset      :   %12.8f Fs\n", _stats->cfo);
    LOGI("    num symbols         :   %u\n", _stats->num_framesyms);
    LOGI("    mod scheme          :   %s (%u bits/symbol)\n",
            modulation_types[_stats->mod_scheme].name,
            _stats->mod_bps);
    LOGI("    validity check      :   %s\n", crc_scheme_str[_stats->check][0]);
