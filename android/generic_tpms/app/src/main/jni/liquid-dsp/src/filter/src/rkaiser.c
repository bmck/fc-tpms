                           ...
                           float * _h)
{
    // validate input
    if (_k < 2) {
        LOGE("error: liquid_firdes_rkaiser(), k must be at least 2\n");
        exit(1);
    } else if (_m < 1) {
        LOGE("error: liquid_firdes_rkaiser(), m must be at least 1\n");
        exit(1);
    } else if (_beta <= 0.0f || _beta >= 1.0f) {
        LOGE("error: liquid_firdes_rkaiser(), beta must be in (0,1)\n");
        exit(1);
    } else if (_dt < -1.0f || _dt > 1.0f) {
        LOGE("error: liquid_firdes_rkaiser(), dt must be in [-1,1]\n");
        exit(1);
    }
                            ...
                            float * _h)
{
    // validate input
    if (_k < 2) {
        LOGE("error: liquid_firdes_arkaiser(), k must be at least 2\n");
        exit(1);
    } else if (_m < 1) {
        LOGE("error: liquid_firdes_arkaiser(), m must be at least 1\n");
        exit(1);
    } else if (_beta <= 0.0f || _beta >= 1.0f) {
        LOGE("error: liquid_firdes_arkaiser(), beta must be in (0,1)\n");
        exit(1);
    } else if (_dt < -1.0f || _dt > 1.0f) {
        LOGE("error: liquid_firdes_arkaiser(), dt must be in [-1,1]\n");
        exit(1);
    }
...
#if DEBUG_RKAISER
    LOGI("rho-hat : %12.8f (compare to %12.8f)\n", rho_hat, rkaiser_approximate_rho(_m,_beta));
    LOGI("fc      : %12.8f\n", fc);
    LOGI("delta-f : %12.8f\n", del);
    LOGI("As      : %12.8f dB\n", As);
    LOGI("alpha   : %12.8f\n", kaiser_beta_As(As));
...
{
    if ( _m < 1 ) {
        LOGE("error: rkaiser_approximate_rho(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: rkaiser_approximate_rho(): beta must be in [0,1]\n");
        exit(1);
    } else;
...
{
    if ( _k < 1 ) {
        LOGE("error: liquid_firdes_rkaiser_bisection(): k must be greater than 0\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_rkaiser_bisection(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_rkaiser_bisection(): beta must be in [0,1]\n");
        exit(1);
    } else;
    ...
    if (y1 > y0 || y1 > y2)
        LOGE("warning: liquid_firdes_rkaiser_bisection(): bounding region is ill-conditioned\n");
    ...
    LOGI("clear all;\n");
    LOGI("close all;\n");
    LOGI("x = [%12.4e %12.4e %12.4e];\n", x0, x1, x2);
    LOGI("y = [%12.4e %12.4e %12.4e];\n", y0, y1, y2);
#endif
    for (p=0; p<pmax; p++) {
...
#if DEBUG_RKAISER
        LOGI("x = [x %12.4e %12.4e];\n", xa, xb);
        LOGI("y = [y %12.4e %12.4e];\n", ya, yb);
#endif
        ...
        LOGI("  %4u : rho=%12.8f, isi=%12.6f dB\n", p+1, x_hat, 20*log10f(y_hat));
#endif
    };
#if DEBUG_RKAISER
    LOGI("figure;\n");
    LOGI("plot(x,20*log10(y),'x');\n");
    fclose(fid);
    LOGI("results written to rkaiser_debug.m\n");
#endif
...
{
    if ( _k < 1 ) {
        LOGE("error: liquid_firdes_rkaiser_quadratic(): k must be greater than 0\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_rkaiser_quadratic(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_rkaiser_quadratic(): beta must be in [0,1]\n");
        exit(1);
    } else;
    ...
    for (p=0; p<pmax; p++) {
...
#if DEBUG_RKAISER
        LOGI("x = [x %12.4e %12.4e %12.4e];\n", x0, x1, x2);
        LOGI("y = [y %12.4e %12.4e %12.4e];\n", y0, y1, y2);
        //LOGI("  %4u : [%8.4f %8.4f %8.4f] rho=%12.8f, isi=%12.6f dB\n", p+1, x0, x1, x2, x1, 20*log10f(y1));
        LOGI("  %4u : [%8.4f,%8.2f] [%8.4f,%8.2f] [%8.4f,%8.2f]\n",
                p+1,
                x0, 20*log10f(y0),
                x1, 20*log10f(y1),
                x2, 20*log10f(y2));
        ...
        if (x_hat < x0 || x_hat > x2) {
            //LOGE("warning: liquid_firdes_rkaiser_quadratic(), quadratic minimum outside boundary\n");
            break;
        }
                                         ...
                                         float * _h)
{
    // validate input
    if (_rho < 0.0f) {
        LOGE("warning: liquid_firdes_rkaiser_internal_isi(), rho < 0\n");
    } else if (_rho > 1.0f) {
        LOGE("warning: liquid_firdes_rkaiser_internal_isi(), rho > 1\n");
    }
