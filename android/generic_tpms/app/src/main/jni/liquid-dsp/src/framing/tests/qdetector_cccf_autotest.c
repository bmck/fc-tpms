...
void qdetector_cccf_runtest_linear(unsigned int _sequence_len)
{
    ...
    if (liquid_autotest_verbose) {
        // print results
        LOGI("\n");
        LOGI("frame detected  :   %s\n", frame_detected ? "yes" : "no");
        LOGI("  sample index  : %8u, actual=%8u (error=%8d)\n", sample_index, buf_len, (int)sample_index - (int)buf_len);
        LOGI("  gamma hat     : %8.3f, actual=%8.3f (error=%8.3f)\n",            gamma_hat, gamma, gamma_hat - gamma);
        LOGI("  tau hat       : %8.3f, actual=%8.3f (error=%8.3f) samples\n",    tau_hat,   tau,   tau_hat   - tau  );
        LOGI("  dphi hat      : %8.5f, actual=%8.5f (error=%8.5f) rad/sample\n", dphi_hat,  dphi,  dphi_hat  - dphi );
        LOGI("  phi hat       : %8.5f, actual=%8.5f (error=%8.5f) radians\n",    phi_hat,   phi,   phi_hat   - phi  );
        LOGI("\n");
    }
