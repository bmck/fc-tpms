                             ...
                             unsigned int _num_traits)
{
    ...
    // validate input
    if (q->num_traits < 1) {
        LOGE("error: chromosome_create(), must have at least one trait\n");
        exit(1);
    }
    ...
    for (i=0; i<q->num_traits; i++) {
        ...
        if (q->bits_per_trait[i] > LIQUID_CHROMOSOME_MAX_SIZE) {
            LOGE("error: chromosome_create(), bits/trait cannot exceed %u\n", LIQUID_CHROMOSOME_MAX_SIZE);
            exit(1);
        }
                                   ...
                                   unsigned int _bits_per_trait)
{
    // validate input
    if (_num_traits == 0) {
        LOGE("error: chromosome_create_basic(), must have at least one trait\n");
        exit(1);
    }
...
void chromosome_printf(chromosome _q)
{
    ...
    LOGI("chromosome: ");
    for (i=0; i<_q->num_traits; i++)
        LOGI("%6.3f", chromosome_valuef(_q,i));
    LOGI("\n");
}
                     ...
                     unsigned int * _v)
{
    ...
    for (i=0; i<_c->num_traits; i++) {
        if (_v[i] >= _c->max_value[i]) {
            LOGE("error: chromosome_init(), value exceeds maximum\n");
            exit(1);
        }
        _c->traits[i] = _v[i];
    }
                      ...
                      float * _v)
{
    ...
    for (i=0; i<_c->num_traits; i++) {
        if (_v[i] > 1.0f || _v[i] < 0.0f) {
            LOGE("error: chromosome_initf(), value must be in [0,1]\n");
            exit(1);
        }
                       ...
                       unsigned int _index)
{
    if (_index >= _q->num_bits) {
        LOGE("error: chromosome_mutate(), maximum index exceeded\n");
        exit(1);
    }
                          ...
                          unsigned int _threshold)
{
    if (_threshold > _q->num_bits) {
        LOGE("error: chromosome_crossover(), maximum index exceeded\n");
        exit(1);
    }
                        ...
                        unsigned int _index)
{
    if (_index > _q->num_traits) {
        LOGE("error: chromosome_valuef(), trait index exceeded\n");
        exit(1);
    }
...
{
    if (_index > _q->num_traits) {
        LOGE("error: chromosome_value(), trait index exceeded\n");
        exit(1);
    }
