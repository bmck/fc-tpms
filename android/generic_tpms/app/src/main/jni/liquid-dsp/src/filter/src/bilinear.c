                  ...
                  float complex * _a)
{
    ...
    LOGI("bilinear(zpk), numerator:\n");
    for (i=0; i<nb; i++)
        LOGI("  b[%3u] = %12.8f + j*%12.8f\n", i, crealf(pb[i]), cimagf(pb[i]));

    LOGI("bilinear(zpk), denominator:\n");
    for (i=0; i<na; i++)
        LOGI("  a[%3u] = %12.8f + j*%12.8f\n", i, crealf(pa[i]), cimagf(pa[i]));
#endif
                 ...
                 float complex * _ad)
{
    if (_b_order > _a_order) {
        LOGE("error: bilinear_nd(), numerator order cannot be higher than denominator\n");
        exit(1);
    }

#if LIQUID_DEBUG_BILINEAR_PRINT
    LOGI("***********************************\n");
    LOGI("bilinear(nd), numerator order   : %u\n", _b_order);
    LOGI("bilinear(nd), denominator order : %u\n", _a_order);
#endif
