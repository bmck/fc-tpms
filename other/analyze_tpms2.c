...
int analyze_file(char *src_filename) {
	...
	LOGI("%s: (%s:%d) FINAL symbols not found", src_name, __FILE__, __LINE__);
	return 0;
}
...
int *get_packet() { //char *src_filename) {
	...
	const int pkt_size = SYMBOLS_PER_MSG;
...
bool is_valid_prelude(unsigned long file_loc) {
	...
	do {
		...
		if (count > MAX_PRELUDE_HF_SAMPLES)  {
			LOGI("%s: (%s:%d) ----> Excessively long HF phase of prelude found", src_name, __FILE__, __LINE__);
			return false;
		}
	...
	} while ((total_samples_in < prelude_hf_start + MIN_PRELUDE_HF_SAMPLES) ||
	...
	do {
		...
		if (count > MAX_PRELUDE_LF_SAMPLES)  {
			LOGI("%s: (%s:%d) ----> Excessively long LF phase of prelude found", src_name, __FILE__, __LINE__);
			return false;
		}
	...
	} while ((total_samples_in < prelude_lf_start + MIN_PRELUDE_LF_SAMPLES) ||
	...
	if (high_freq <= low_freq) {
		LOGI("%s: (%s:%d) ----> Invalid high_freq (%lf) and low_freq (%lf) combination found", src_name, __FILE__, __LINE__, high_freq, low_freq);
		return false;
	}
	...
	if (samples_per_sym < MIN_SAMPLES_PER_SYMBOL) {
		LOGI("%s: (%s:%d) ----> Invalid samples/symbol found %u < %d", src_name, __FILE__, __LINE__, samples_per_sym, (int)MIN_SAMPLES_PER_SYMBOL);
		return false;
	}
...
int get_bit(unsigned long bit_start, unsigned long bit_end) {
	...
	int transition_point = scan_samples_for_eob_transition(samples_buffer, num_samples * 1.1);
	actual_bit_end = (transition_point == -1) ? bit_end : (transition_point - 1 + bit_start);
	...
	LOGI("%s: (%s:%d) ----------> num_samples = %d, b = %d, confidence = %f", src_name, __FILE__, __LINE__, num_samples, b, bit_confidence);

	b = (b > 0) ? HIGHBIT : LOWBIT;
	free(samples_buffer);
	free(samples);
	LOGI("%s: (%s:%d) ----------> bit returned = %d", src_name, __FILE__, __LINE__, b);
	return b;
}
...
int scan_samples_for_eob_transition(float complex *samples, int num_samples) {
	...
	for ( ; i < num_samples; i++) {
		phi = cargf(samples[i]);
		later_delta_phi = (DELTA_PHI_DEFN);
		prev_phi = phi;
...
char *get_hex_address_str(char *returned_url) {
	...
	sprintf(addr_str, "%lX", get_dec_address_val());
...
double get_pressure_psi() {
	double psi = get_pressure_kpa() * 0.145037738;
