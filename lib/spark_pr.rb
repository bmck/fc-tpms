...
#
# taken from https://github.com/madrobby/spark_pr
#
#
...
module Spark
  ...
  def Spark.normalize( arr, type = :linear )
    ...
    arr.map {|v| fac.zero? ? 0 : ((v-adj).quo(fac) rescue 0) }
  end

  def Spark.process_options( options )
    o = options.inject({}) do |oo, (key, value)|
      oo[key.to_sym] = value ; oo
    end
  ...
  def Spark.smooth( results, options = {} )
    ...
    # Rails.logger.debug_w_fn "results = #{results}, options = #{options.inspect}"
    o = {
      step: 2,
      height: 14,
      has_min: false,
    ...
    o[:width] ||= ([results.size-1, 4].max)*o[:step] + 5
    # Rails.logger.debug_w_fn "o = #{o.inspect}"
