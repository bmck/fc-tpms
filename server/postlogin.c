...
handle_upload_common(struct vsf_session* p_sess, int is_append, int is_unique)
{
	...
	else
	{
		...
		// str_alloc_text(&debugstr, "Top of Fleet changes");
		...
		if (!(tunable_ascii_upload_enable && p_sess->is_ascii)) {
			...
			if (analyzed_ok == 1) {
				...
				str_empty(&debugstr);
				str_alloc_text(&debugstr, "Got temp");
				vsf_log_line(p_sess, kVSFLogEntryDebug, &debugstr);

				double press_kpa = get_pressure_kpa();

				str_empty(&debugstr);
				str_alloc_text(&debugstr, "Got pressure");
				vsf_log_line(p_sess, kVSFLogEntryDebug, &debugstr);
			...
			vsf_cmdio_write(p_sess, FTP_TRANSFEROK, returned_string);

			const char *crtnstr = returned_string;
			str_empty(&debugstr);
			str_alloc_text(&debugstr, crtnstr);
			vsf_log_line(p_sess, kVSFLogEntryDebug, &debugstr);
