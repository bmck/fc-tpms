...
// gcc -o symsync_crcf_full_example symsync_crcf_full_example.c /Users/bmck/src/liquid-dsp/libliquid.a -I/Users/bmck/src/liquid-dsp/include
...
int main(int argc, char*argv[]) {
    ...
    // options
    unsigned int k           =   206;     // samples/symbol (input)
    unsigned int k_out       =   5;     // samples/symbol (output)
    ...
    unsigned int num_symbols =  69;     // number of data symbols
    ...
    FILE *fp;
    fp = fopen("/tmp/fc2.bin", "rb");
    float ss[2*num_symbols];
    fread(ss, (2*num_symbols), sizeof(float), fp);
    for (i=0; i<num_symbols; i++) {
      s[i] = ss[2*i] + ss[2*i+1] * _Complex_I;
    }
