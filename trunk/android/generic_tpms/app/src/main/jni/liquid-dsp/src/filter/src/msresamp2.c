                               ...
                               float        _As)
{
    // validate input
    if (_num_stages > 10) {
        LOGE("error: msresamp2_%s_create(), number of stages should not exceed 10\n", EXTENSION_FULL);
        exit(1);
    }

    // ensure cut-off frequency is valid
    if ( _fc <= 0.0f || _fc >= 0.5f ) {
        LOGE("error: msresamp2_%s_create(), cut-off frequency must be in (0,0.5)\n", EXTENSION_FULL);
        exit(1);
    } else if ( _fc > 0.45f ) {
        LOGE("warning: msresamp2_%s_create(), cut-off frequency greater than 0.45\n", EXTENSION_FULL);
        LOGE("    >> truncating to 0.45\n");
        _fc = 0.45f;
    }

    // check center frequency
    if ( _f0 != 0. ) {
        LOGE("warning: msresamp2_%s_create(), non-zero center frequency not yet supported\n", EXTENSION_FULL);
        _f0 = 0.;
    }
...
{
    LOGI("multi-stage half-band resampler:\n");
    LOGI("    type                    : %s\n", _q->type == LIQUID_RESAMP_DECIM ? "decimator" : "interpolator");
    LOGI("    number of stages        : %u stage%s\n", _q->num_stages, _q->num_stages == 1 ? "" : "s");
    LOGI("    cut-off frequency, fc   : %12.8f Fs\n",  _q->fc);
    LOGI("    center frequency, f0    : %12.8f Fs\n",  _q->f0);
    LOGI("    stop-band attenuation   : %.2f dB\n",    _q->As);

    // print each stage
    unsigned int i;
    for (i=0; i<_q->num_stages; i++) {
        LOGI("    stage[%2u]  {m=%3u, As=%6.2f dB, fc=%6.3f, f0=%6.3f}\n",
