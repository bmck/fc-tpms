                       ...
                       unsigned int *  _M_S0)
{
    ...
    if (M_S0 == 0) {
        LOGE("error: ofdmframe_init_S0(), no subcarriers enabled; check allocation\n");
        exit(1);
    }
                       ...
                       unsigned int *  _M_S1)
{
    ...
    // ensure at least one subcarrier was enabled
    if (M_S1 == 0) {
        LOGE("error: ofdmframe_init_S1(), no subcarriers enabled; check allocation\n");
        exit(1);
    }
                                   ...
                                   unsigned char * _p)
{
    // validate input
    if (_M < 6) {
        LOGE("warning: ofdmframe_init_default_sctype(), less than 4 subcarriers\n");
    }
                               ...
                               unsigned int * _M_data)
{
    ...
    for (i=0; i<_M; i++) {
        ...
        if (_p[i] == OFDMFRAME_SCTYPE_NULL)
            ...
            LOGE("error: ofdmframe_validate_sctype(), invalid subcarrier type (%u)\n", _p[i]);
            exit(1);
        }
    }
                            ...
                            unsigned int    _M)
{
    ...
    for (i=0; i<_M; i++) {
        ...
        switch (_p[k]) {
        case OFDMFRAME_SCTYPE_NULL:     LOGI(".");    break;
        case OFDMFRAME_SCTYPE_PILOT:    LOGI("|");    break;
        case OFDMFRAME_SCTYPE_DATA:     LOGI("+");    break;
        default:
            LOGE("error: ofdmframe_print_default_sctype(), invalid subcarrier type\n");
            exit(1);
        }
    }
