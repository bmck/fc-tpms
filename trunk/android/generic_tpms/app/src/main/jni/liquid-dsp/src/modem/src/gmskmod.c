...
{
    if (_k < 2) {
        LOGE("error: gmskmod_create(), samples/symbol must be at least 2\n");
        exit(1);
    } else if (_m < 1) {
        LOGE("error: gmskmod_create(), symbol delay must be at least 1\n");
        exit(1);
    } else if (_BT <= 0.0f || _BT >= 1.0f) {
        LOGE("error: gmskmod_create(), bandwidth/time product must be in (0,1)\n");
        exit(1);
    }
