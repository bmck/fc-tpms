                   ...
                   unsigned int * _num_factors)
{
    ...
    if (n > 1 && num_factors == LIQUID_MAX_FACTORS) {
        LOGE("error, liquid_factor(), could not factor %u in %u numbers\n", _n, LIQUID_MAX_FACTORS);
        exit(1);
    }
                          ...
                          unsigned int * _num_factors)
{
    ...
    if (n > 1 && num_factors == LIQUID_MAX_FACTORS) {
        LOGE("error, liquid_unqiue_factor(), could not factor %u in %u numbers\n", _n, LIQUID_MAX_FACTORS);
        exit(1);
    }
...
unsigned int liquid_primitive_root_prime(unsigned int _n)
{
    ...
    LOGI("found %u unique factors of n-1 = %u\n", num_unique_factors, _n-1);
