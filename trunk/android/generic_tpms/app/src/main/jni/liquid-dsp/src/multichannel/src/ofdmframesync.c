                                   ...
                                   void *                 _userdata)
{
    ...
    // validate input
    if (_M < 8) {
        LOGE("warning: ofdmframesync_create(), less than 8 subcarriers\n");
    } else if (_M % 2) {
        LOGE("error: ofdmframesync_create(), number of subcarriers must be even\n");
        exit(1);
    } else if (_cp_len > _M) {
        LOGE("error: ofdmframesync_create(), cyclic prefix length cannot exceed number of subcarriers\n");
        exit(1);
    }
    q->M = _M;
    q->cp_len = _cp_len;
    ...
    if ( (q->M_pilot + q->M_data) == 0) {
        LOGE("error: ofdmframesync_create(), must have at least one enabled subcarrier\n");
        exit(1);
    } else if (q->M_data == 0) {
        LOGE("error: ofdmframesync_create(), must have at least one data subcarriers\n");
        exit(1);
    } else if (q->M_pilot < 2) {
        LOGE("error: ofdmframesync_create(), must have at least two pilot subcarriers\n");
        exit(1);
    }
...
{
    LOGI("ofdmframesync:\n");
    LOGI("    num subcarriers     :   %-u\n", _q->M);
    LOGI("    cyclic prefix len   :   %-u\n", _q->cp_len);
    //LOGI("    taper len           :   %-u\n", _q->taper_len);
}
...
void ofdmframesync_execute_seekplcp(ofdmframesync _q)
{
    ...
    LOGI(" - gain=%12.3f, rssi=%12.8f, s_hat=%12.4f <%12.8f>, tau_hat=%8.3f\n",
            sqrt(g),
            -10*log10(g),
...
#if DEBUG_OFDMFRAMESYNC_PRINT
        LOGI("could not find S1 symbol. bailing...\n");
#endif
        ofdmframesync_reset(_q);
    }
                                   ...
                                   unsigned int _ntaps)
{
    ...
    if (_ntaps == 0 || _ntaps > _q->M) {
        LOGE( "error: ofdmframesync_estimate_eqgain(), ntaps must be in [1,M]\n");
        exit(1);
    }
    ...
    for (i=0; i<_q->M; i++) {
        ...
        if (cabsf(w0) < 1e-4f) {
            LOGE("error: ofdmframesync_estimate_eqgain(), weighting factor is zero\n");
            w0 = 1.0f;
        }
        _q->G[i] = G_hat / w0;
    }
                                        ...
                                        unsigned int _order)
{
    ...
    for (i=0; i<_q->M; i++) {
        ...
        if (_q->p[k] != OFDMFRAME_SCTYPE_NULL) {
            if (n == N) {
                LOGE( "error: ofdmframesync_estimate_eqgain_poly(), pilot subcarrier mismatch\n");
                exit(1);
            }
            // store resulting...
...
#if 0
    for (i=0; i<N; i++)
        LOGI("x(%3u) = %12.8f; y_abs(%3u) = %12.8f; y_arg(%3u) = %12.8f;\n",
                i+1, x_freq[i],
                i+1, y_abs[i],
                i+1, y_arg[i]);
...
void ofdmframesync_rxsymbol(ofdmframesync _q)
{
    ...
    for (i=0; i<_q->M; i++) {
        ...
        if (_q->p[k]==OFDMFRAME_SCTYPE_PILOT) {
            if (n == _q->M_pilot) {
                LOGE("warning: ofdmframesync_rxsymbol(), pilot subcarrier mismatch\n");
                return;
            }
            pilot = (msequence_advance(_q->ms_pilot) ? 1.0f : -1.0f);
#if 0
            LOGI("pilot[%3u] = %12.4e + j*%12.4e (expected %12.4e + j*%12.4e)\n",
                    k,
    ...
    for (i=0; i<_q->M_pilot; i++)
        LOGI("x_phase(%3u) = %12.8f; y_phase(%3u) = %12.8f;\n", i+1, x_phase[i], i+1, y_phase[i]);
    LOGI("poly : p0=%12.8f, p1=%12.8f\n", p_phase[0], p_phase[1]);
#endif
}
...
#else
    LOGE("ofdmframesync_debug_enable(): compile-time debugging disabled\n");
#endif
}
                               ...
                               const char * _filename)
{
#if DEBUG_OFDMFRAMESYNC
    if (!_q->debug_objects_created) {
        LOGE("error: ofdmframe_debug_print(), debugging objects don't exist; enable debugging first\n");
        return;
    }
    ...
    LOGI("s1 = [];\n");
    for (i=0; i<_q->M; i++)
        LOGI("s1(%3u) = %12.4e + j*%12.4e;\n", i+1, crealf(_q->s1[i]), cimagf(_q->s1[i]));


    // write agc_rssi
    LOGI("\n\n");
    LOGI("agc_rssi = zeros(1,%u);\n", DEBUG_OFDMFRAMESYNC_BUFFER_LEN);
    windowf_read(_q->debug_rssi, &r);
    for (i=0; i<DEBUG_OFDMFRAMESYNC_BUFFER_LEN; i++)
        LOGI("agc_rssi(%4u) = %12.4e;\n", i+1, r[i]);
    LOGI("agc_rssi = filter([0.00362168 0.00724336 0.00362168],[1 -1.82269490 0.83718163],agc_rssi);\n");
    LOGI("agc_rssi = 10*log10( agc_rssi );\n");
    LOGI("figure;\n");
    LOGI("plot(agc_rssi)\n");
    LOGI("ylabel('RSSI [dB]');\n");
    ...
    LOGI("S1 = zeros(1,M);\n");
    for (i=0; i<_q->M; i++) {
        LOGI("S0(%3u) = %12.8f + j*%12.8f;\n", i+1, crealf(_q->S0[i]), cimagf(_q->S0[i]));
        LOGI("S1(%3u) = %12.8f + j*%12.8f;\n", i+1, crealf(_q->S1[i]), cimagf(_q->S1[i]));
    }
    ...
    LOGI("G0     = zeros(1,M);\n");
    LOGI("G1     = zeros(1,M);\n");
    LOGI("G_hat  = zeros(1,M);\n");
    LOGI("G      = zeros(1,M);\n");
    for (i=0; i<_q->M; i++) {
        LOGI("G0(%3u)    = %12.8f + j*%12.8f;\n", i+1, crealf(_q->G0[i]),   cimagf(_q->G0[i]));
        LOGI("G1(%3u)    = %12.8f + j*%12.8f;\n", i+1, crealf(_q->G1[i]),   cimagf(_q->G1[i]));
        LOGI("G_hat(%3u) = %12.8f + j*%12.8f;\n", i+1, crealf(_q->G_hat[i]),cimagf(_q->G_hat[i]));
        LOGI("G(%3u)     = %12.8f + j*%12.8f;\n", i+1, crealf(_q->G[i]),    cimagf(_q->G[i]));
    }
    LOGI("f = [0:(M-1)];\n");
    LOGI("figure;\n");
    LOGI("subplot(2,1,1);\n");
    LOGI("  plot(f, fftshift(abs(G_hat)),'sb',...\n");
    LOGI("       f, fftshift(abs(G)),'-k','LineWidth',2);\n");
    LOGI("  grid on;\n");
    LOGI("  xlabel('subcarrier index');\n");
    LOGI("  ylabel('gain estimate (mag)');\n");
    LOGI("subplot(2,1,2);\n");
    LOGI("  plot(f, fftshift(arg(G_hat).*[abs(G0) > 1e-3]),'sb',...\n");
    LOGI("       f, fftshift(arg(G)),'-k','LineWidth',2);\n");
    LOGI("  grid on;\n");
    LOGI("  xlabel('subcarrier index');\n");
    LOGI("  ylabel('gain estimate (phase)');\n");

    // write pilot response
    LOGI("\n\n");
    LOGI("px = zeros(1,M_pilot);\n");
    LOGI("py = zeros(1,M_pilot);\n");
    for (i=0; i<_q->M_pilot; i++) {
        LOGI("px(%3u) = %12.8f;\n", i+1, _q->px[i]);
        LOGI("py(%3u) = %12.8f;\n", i+1, _q->py[i]);
    }
    LOGI("p_phase(1) = %12.8f;\n", _q->p_phase[0]);
    LOGI("p_phase(2) = %12.8f;\n", _q->p_phase[1]);
    ...
    LOGI("subplot(3,1,1);\n");
    LOGI("  plot(px, py, 'sb',...\n");
    LOGI("       fp, polyval(p_phase, fp), '-k');\n");
    LOGI("  grid on;\n");
    LOGI("  legend('pilots','polyfit',0);\n");
    LOGI("  xlabel('subcarrier');\n");
    LOGI("  ylabel('phase');\n");
    LOGI("subplot(3,1,2);\n");
    LOGI("  plot(1:length(p0), p0);\n");
    LOGI("  grid on;\n");
    LOGI("  ylabel('p0 (phase offset)');\n");
    LOGI("subplot(3,1,3);\n");
    LOGI("  plot(1:length(p1), p1);\n");
    LOGI("  grid on;\n");
    LOGI("  ylabel('p1 (phase slope)');\n");

    // write frame symbols
    LOGI("framesyms = zeros(1,n);\n");
    windowcf_read(_q->debug_framesyms, &rc);
    for (i=0; i<DEBUG_OFDMFRAMESYNC_BUFFER_LEN; i++)
        LOGI("framesyms(%4u) = %12.4e + j*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));
    LOGI("figure;\n");
    LOGI("plot(real(framesyms), imag(framesyms), 'x');\n");
    LOGI("xlabel('I');\n");
    LOGI("ylabel('Q');\n");
    LOGI("axis([-1 1 -1 1]*1.6);\n");
    LOGI("axis square;\n");
    LOGI("grid on;\n");

    fclose(fid);
    LOGI("ofdmframesync/debug: results written to '%s'\n", _filename);
#else
    LOGE("ofdmframesync_debug_print(): compile-time debugging disabled\n");
#endif
}
