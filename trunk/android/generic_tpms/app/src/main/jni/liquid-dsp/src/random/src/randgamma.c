                 ...
                 float _beta)
{
    // validate input
    if (_alpha <= 0.0f) {
        LOGE("error: randgammaf(), alpha must be greater than zero\n");
        exit(1);
    } else if (_beta <= 0.0f) {
        LOGE("error: randgammaf(), beta must be greater than zero\n");
        exit(1);
    }
                     ...
                     float _beta)
{
    // validate input
    if (_alpha <= 0.0f) {
        LOGE("error: randgammaf_pdf(), alpha must be greater than zero\n");
        exit(1);
    } else if (_beta <= 0.0f) {
        LOGE("error: randgammaf_pdf(), beta must be greater than zero\n");
        exit(1);
    }
                     ...
                     float _beta)
{
    // validate input
    if (_alpha <= 0.0f) {
        LOGE("error: randgammaf_cdf(), alpha must be greater than zero\n");
        exit(1);
    } else if (_beta <= 0.0f) {
        LOGE("error: randgammaf_cdf(), beta must be greater than zero\n");
        exit(1);
    }
...
float randgammaf_delta(float _delta)
{
    ...
    if ( _delta < 0.0f || _delta >= 1.0f ) {
        LOGE("error: randgammaf_delta(), delta must be in [0,1)\n");
        exit(1);
    }
