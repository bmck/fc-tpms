                               ...
                               unsigned int _num_bits)
{
    // validate input
    if (_num_bits == 0) {
        LOGE("error: quantizer_create(), must have at least one bit/sample\n");
        exit(1);
    }
...
void QUANTIZER(_print)(QUANTIZER() _q)
{
    ...
    LOGI("  compander :   ");
    switch(_q->ctype) {
    case LIQUID_COMPANDER_NONE:     LOGI("none\n");   break;
    case LIQUID_COMPANDER_LINEAR:   LOGI("linear\n"); break;
    case LIQUID_COMPANDER_MULAW:    LOGI("mu-law\n"); break;
    case LIQUID_COMPANDER_ALAW:     LOGI("A-law\n");  break;
    default:
        LOGI("unknown\n");
    }
    LOGI("  num bits  :   %u\n", _q->n);
