...
MODEM() MODEM(_create)(modulation_scheme _scheme)
{
    ...
    case LIQUID_MODEM_ARB:
        LOGE("error: modem_create(), cannot create arbitrary modem (LIQUID_MODEM_ARB) without specifying constellation\n");
        exit(1);

    // unknown modulation scheme
    default:
        LOGE("error: modem_create(), unknown/unsupported modulation scheme : %u\n", _scheme);
        exit(1);
    }
                  ...
                  unsigned int _bits_per_symbol)
{
    if (_bits_per_symbol < 1 ) {
        LOGE("error: modem_init(), modem must have at least 1 bit/symbol\n");
        exit(1);
    } else if (_bits_per_symbol > MAX_MOD_BITS_PER_SYMBOL) {
        LOGE("error: modem_init(), maximum number of bits per symbol exceeded\n");
        exit(1);
    }
...
void MODEM(_init_map)(MODEM() _q)
{
    // validate input
    if (_q->symbol_map == NULL) {
        LOGE("error: modem_init_map(), symbol map array has not been allocated\n");
        exit(1);
    } else if (_q->M == 0 || _q->M > (1<<MAX_MOD_BITS_PER_SYMBOL)) {
        LOGE("error: modem_init_map(), constellation size is out of range\n");
        exit(1);
    } else if (_q->modulate_func == NULL) {
        LOGE("error: modem_init_map(), modulation function has not been initialized\n");
        exit(1);
    }
                      ...
                      TC * _y)
{
    // validate input
    if (_symbol_in >= _q->M) {
        LOGE("error: modem_modulate(), input symbol exceeds constellation size\n");
        exit(1);
    }
...
{
    if (_symbol_in >= _q->M) {
        LOGE("error: modem_modulate_table(), input symbol exceeds maximum\n");
        exit(1);
    } else if (_q == NULL) {
        LOGE("error: modem_modulate_table(), symbol table not initialized\n");
        exit(1);
    }
                              ...
                              unsigned int _p)
{
    ...
    if (_p > (_q->M-1)) {
        LOGE("error: modem_demodsoft_gentab(), requesting too many neighbors\n");
        exit(1);
    }
    ...
    for (i=0; i<M; i++) {
        ...
        for (k=0; k<_p; k++) {
            ...
            if (_q->demod_soft_neighbors[i*_p + k] < M)
                LOGI("(%6.4f)", cabsf( c[i]-c[_q->demod_soft_neighbors[i*_p+k]] ));
            else
                LOGI("        ");
        }
        LOGI("\n");
    ...
    // print c-type array
    LOGI("\n");
    LOGI("// %s%u soft demodulation nearest neighbors (p=%u)\n", modulation_types[_q->scheme].name, M, _p);
    LOGI("const unsigned char %s%u_demod_soft_neighbors[%u] = {\n", modulation_types[_q->scheme].name, M, _p*M);
    LOGI("    ");
    for (i=0; i<M; i++) {
        for (k=0; k<_p; k++) {
            LOGI("%4u", _q->demod_soft_neighbors[i*_p+k]);
            if (k != _p-1) LOGI(",");
        }
        if (i != M-1) {
            LOGI(",   // ");
