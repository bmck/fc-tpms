...
unsigned int fec_hamming3126_encode_symbol(unsigned int _sym_dec)
{
    // validate input
    if (_sym_dec >= (1<<26)) {
        LOGE("error, fec_hamming_encode(), input symbol too large\n");
        exit(1);
    }
    ...
    LOGI("parity bits (p1,p2,p4,p8,p16) : (%1u,%1u,%1u,%1u,%1u)\n", p1, p2, p4, p8, p16);
#endif
...
unsigned int fec_hamming3126_decode_symbol(unsigned int _sym_enc)
{
    // validate input
    if (_sym_enc >= (1<<31)) {
        LOGE("error, fec_hamming_decode(), input symbol too large\n");
        exit(1);
    }
...
#if DEBUG_FEC_HAMMING
    LOGI("syndrome bits (s1,s2,s4,s8,16) : (%1u,%1u,%1u,%1u,%1u)\n", s1, s2, s4, s8, s16);
    LOGI("syndrome z : %u\n", z);
#endif
    ...
    if (z) {
        if (z > 31) {
            ...
            LOGE("warning, fec_hamming3126_decode_symbol(), syndrome index exceeds maximum\n");
        } else {
            //LOGI("error detected!\n");
            _sym_enc ^= 1 << (31-z);
        }
