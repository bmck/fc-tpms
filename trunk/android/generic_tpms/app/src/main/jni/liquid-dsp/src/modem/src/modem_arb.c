                               ...
                               unsigned int _M)
{
    ...
    if ( (1<<m) != _M ) {
        ...
        LOGE("error: modem_create_arbitrary(), input constellation size must be power of 2\n");
        exit(1);
    }
...
{
    if (_sym_in >= _q->M) {
        LOGE("error: modulate_arb(), input symbol exceeds maximum\n");
        exit(1);
    }
...
{
    //LOGI("modem_demodulate_arb() invoked with I=%d, Q=%d\n", x);
                      ...
                      unsigned int _len)
{
    ...
    if (_q->scheme != LIQUID_MODEM_ARB) {
        LOGE("error: modem_arb_init(), modem is not of arbitrary type\n");
        exit(1);
    } else if (_len != _q->M) {
        LOGE("error: modem_arb_init(), array sizes do not match\n");
        exit(1);
    }
#endif
                           ...
                           char *  _filename)
{
    ...
    if (fid == NULL) {
        LOGE("error: modem_arb_init_file(), could not open file\n");
        exit(1);
    }
    ...
    for (i=0; i<_q->M; i++) {
        if ( feof(fid) ) {
            LOGE("error: modem_arb_init_file(), premature EOF for '%s'\n", _filename);
            exit(1);
        }
        ...
        if (results < 2) {
            LOGE("error: modem_arb_init_file(), unable to parse line\n");
            exit(1);
        }
    }
