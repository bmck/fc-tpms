...
{
    if ( _k < 2 ) {
        LOGE("error: liquid_firdes_hM3(): k must be greater than 1\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_hM3(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_hM3(): beta must be in [0,1]\n");
        exit(1);
    } else;
