                                 ...
                                 unsigned char * _sym_dec)
{
    ...
    // validate input
    if (_sym_enc[0] >= (1<<6)) {
        LOGE("warning, fec_secded2216_decode_symbol(), input symbol too large\n");
    }
#endif
    ...
    if (syndrome_flag == 1) {
        LOGI("secded2216_decode_symbol(): single error detected!\n");
    } else if (syndrome_flag == 2) {
        LOGI("secded2216_decode_symbol(): no match found (multiple errors detected)\n");
    }
#endif
