                           ...
                           float        _dphi)
{
    ...
    for (i=0; i<num_samples; i++) {
        ...
        if (detected) {
            ...
            if (liquid_autotest_verbose) {
                LOGI("****** preamble found, tau_hat=%8.6f, dphi_hat=%8.6f, gamma_hat=%8.6f\n",
    ...
    if (liquid_autotest_verbose) {
        LOGI("detector autotest [%3u]: signal detected? %s\n", _n, signal_detected ? "yes" : "no");
        LOGI("    dphi    :   estimate = %12.6f (expected %12.6f)\n", dphi_hat,  _dphi);
        LOGI("    delay   :   estimate = %12.6f (expected %12.6f)\n", delay_hat, delay);
        LOGI("    gamma   :   estimate = %12.6f (expected %12.6f)\n", gamma_hat, gamma);
    }
