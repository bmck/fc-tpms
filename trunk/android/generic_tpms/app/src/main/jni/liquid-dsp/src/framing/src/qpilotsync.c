                             ...
                             unsigned int _pilot_spacing)
{
    // validate input
    if (_payload_len == 0) {
        LOGE("error: qpilotsync_create(), frame length must be at least 1 symbol\n");
        exit(1);
    } else if (_pilot_spacing < 2) {
        LOGE("error: qpilotsync_create(), pilot spacing must be at least 2 symbols\n");
        exit(1);
    }
    unsigned int i;
                        ...
                        float complex * _payload)
{
...
#if DEBUG_QPILOTSYNC
        LOGI("(%8.4f,%8.4f) = (%8.4f,%8.4f) * conj(%8.4f,%8.4f)\n",
    ...
    for (i=0; i<_q->nfft; i++) {
#if DEBUG_QPILOTSYNC
        LOGI("X(%3u) = %12.8f + 1i*%12.8f; %% %12.8f\n",
