...
unsigned int fec_golay2412_encode_symbol(unsigned int _sym_dec)
{
    // validate input
    if (_sym_dec >= (1<<12)) {
        LOGE("error, fec_golay2412_encode_symbol(), input symbol too large\n");
        exit(1);
    }
...
unsigned int fec_golay2412_decode_symbol(unsigned int _sym_enc)
{
    // validate input
    if (_sym_enc >= (1<<24)) {
        LOGE("error, fec_golay2412_decode_symbol(), input symbol too large\n");
        exit(1);
    }
    ...
    LOGI("s (syndrome vector): "); liquid_print_bitstring(s,12); LOGI("\n");
#endif
    ...
    if (ws <= 3) {
#if DEBUG_FEC_GOLAY2412
        LOGI("    w(s) <= 3: estimating error vector as [s, 0(12)]\n");
#endif
        // set e_hat = [s 0(12)]
        e_hat = (s << 12) & 0xfff000;
    } else {
        ...
        if (s_index >= 0) {
            // vector found!
#if DEBUG_FEC_GOLAY2412
            LOGI("    w(s + p[%2u]) <= 2: estimating error vector as [s+p[%2u],u[%2u]]\n", s_index, s_index, s_index);
#endif
            // NOTE : uj = 1 << (12-j-1)
...
#if DEBUG_FEC_GOLAY2412
            LOGI("s*P: "); liquid_print_bitstring(sP,12); LOGI("\n");
#endif
            ...
            if (wsP == 2 || wsP == 3) {
                // step 5: set e = [0, s*P]
#if DEBUG_FEC_GOLAY2412
                LOGI("    w(s*P) in [2,3]: estimating error vector as [0(12), s*P]\n");
#endif
                e_hat = sP;
            } else {
...
#if DEBUG_FEC_GOLAY2412
                LOGI("    searching for w(s*P + p_i) == 2...\n");
#endif
                ...
                if (sP_index >= 0) {
                    // vector found!
#if DEBUG_FEC_GOLAY2412
                    LOGI("    w(s*P + p[%2u]) == 2: estimating error vector as [u[%2u],s*P+p[%2u]]\n", sP_index, sP_index, sP_index);
#endif
                    // NOTE : uj = 1 << (12-j-1)
                    //      [      uj << 1 2    ] [    sP + p[j]    ]
...
#if DEBUG_FEC_GOLAY2412
    LOGI("r (recevied vector):            "); liquid_print_bitstring(_sym_enc,24); LOGI("\n");
    LOGI("e-hat (estimated error vector): "); liquid_print_bitstring(e_hat,24);    LOGI("\n");
    LOGI("v-hat (estimated tx vector):    "); liquid_print_bitstring(v_hat,24);    LOGI("\n");
#endif
