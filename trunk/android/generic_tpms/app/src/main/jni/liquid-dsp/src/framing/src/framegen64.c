...
void framegen64_print(framegen64 _q)
{
    ...
    LOGI("framegen64 [m=%u, beta=%4.2f]:\n", _q->m, _q->beta);
    LOGI("  preamble/etc.\n");
    LOGI("    * ramp/up symbols       :   %3u\n", _q->m);
    LOGI("    * p/n symbols           :   %3u\n", 64);
    LOGI("    * ramp\\down symbols     :   %3u\n", _q->m);
    LOGI("    * zero padding          :   %3u\n", 12);
    LOGI("  payload\n");
#if 0
...
#endif
    LOGI("    * payload len, uncoded  :   %3u bytes\n", 64);
    LOGI("    * payload len, coded    :   %3u bytes\n", 150);
    LOGI("    * modulation scheme     :   %s\n", modulation_types[LIQUID_MODEM_QPSK].name);
    LOGI("    * payload symbols       :   %3u\n", 600);
    LOGI("    * pilot symbols         :   %3u\n", 30);
    LOGI("  summary\n");
    LOGI("    * total symbols         :   %3u\n", LIQUID_FRAME64_LEN/2);
