                         ...
                         unsigned int    _n)
{
    ...
    for (i=0; i<_n; i++) {
        ...
        case FRAMESYNC64_STATE_RXPAYLOAD:
            ...
            break;
        default:
            LOGE("error: framesync64_exeucte(), unknown/unsupported state\n");
            exit(1);
        }
    }
}
...
#else
    LOGE("framesync64_debug_enable(): compile-time debugging disabled\n");
#endif
}
                             ...
                             const char * _filename)
{
#if DEBUG_FRAMESYNC64
    if (!_q->debug_objects_created) {
        LOGE("error: framesync64_debug_print(), debugging objects don't exist; enable debugging first\n");
        return;
    }
    unsigned int i;
    float complex * rc;
    ...
    LOGI("payload_rx = zeros(1,%u);\n", payload_sym_len);
    rc = _q->payload_rx;
    for (i=0; i<payload_sym_len; i++)
        LOGI("payload_rx(%4u) = %12.4e + j*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    // write payload symbols
    LOGI("payload_syms = zeros(1,%u);\n", payload_sym_len);
    rc = _q->payload_sym;
    for (i=0; i<payload_sym_len; i++)
        LOGI("payload_syms(%4u) = %12.4e + j*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    LOGI("figure;\n");
    LOGI("plot(real(payload_syms),imag(payload_syms),'o');\n");
    ...
    LOGI("framesync64/debug: results written to %s\n", _filename);
#else
    LOGE("framesync64_debug_print(): compile-time debugging disabled\n");
#endif
}
