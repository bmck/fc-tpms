                           ...
                           float * _a)
{
    // validate input
    if (_w <= 0.0f) {
        LOGE("error: iirdes_pll_active_lag(), bandwidth must be greater than 0\n");
        exit(1);
    } else if (_zeta <= 0.0f) {
        LOGE("error: iirdes_pll_active_lag(), damping factor must be greater than 0\n");
        exit(1);
    } else if (_K <= 0.0f) {
        LOGE("error: iirdes_pll_active_lag(), gain must be greater than 0\n");
        exit(1);
    }
