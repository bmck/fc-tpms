                           ...
                           unsigned int _n)
{
    // validate input
    if (_n == 0) {
        LOGE("error: firfilt_%s_create(), filter length must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    }
                                  ...
                                  float        _mu)
{
    // validate input
    if (_n == 0) {
        LOGE("error: firfilt_%s_create_kaiser(), filter length must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    }
                                    ...
                                    float        _mu)
{
    // validate input
    if (_k < 2) {
        LOGE("error: firfilt_%s_create_rnyquist(), filter samples/symbol must be greater than 1\n", EXTENSION_FULL);
        exit(1);
    } else if (_m == 0) {
        LOGE("error: firfilt_%s_create_rnyquist(), filter delay must be greater than 0\n", EXTENSION_FULL);
        exit(1);
    } else if (_beta < 0.0f || _beta > 1.0f) {
        LOGE("error: firfilt_%s_create_rnyquist(), filter excess bandwidth factor must be in [0,1]\n", EXTENSION_FULL);
        exit(1);
    }
