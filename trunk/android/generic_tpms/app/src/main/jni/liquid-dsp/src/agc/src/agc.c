                         ...
                         float _bt)
{
    ...
    if ( _bt < 0 ) {
        LOGE("error: agc_%s_set_bandwidth(), bandwidth must be positive\n", EXTENSION_FULL);
        exit(-1);
    } else if ( _bt > 1.0f ) {
        LOGE("error: agc_%s_set_bandwidth(), bandwidth must less than 1.0\n", EXTENSION_FULL);
        exit(-1);
    }
                            ...
                            float _signal_level)
{
    ...
    if ( _signal_level <= 0 ) {
        LOGE("error: agc_%s_set_signal_level(), bandwidth must be greater than zero\n", EXTENSION_FULL);
        exit(-1);
    }
                    ...
                    float _gain)
{
    ...
    if ( _gain <= 0 ) {
        LOGE("error: agc_%s_set_gain(), gain must be greater than zero\n", EXTENSION_FULL);
        exit(-1);
    }
                ...
                unsigned int _n)
{
    ...
    if ( _n == 0 ) {
        LOGE("error: agc_%s_init(), number of samples must be greater than zero\n", EXTENSION_FULL);
        exit(-1);
    }
