                            ...
                            float *             _h)
{
    // validate input
    if ( _k < 1 ) {
        LOGE("error: liquid_firdes_fnyquist(): k must be greater than 0\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_fnyquist(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_fnyquist(): beta must be in [0,1]\n");
        exit(1);
    } else;
    ...
    case LIQUID_FIRFILT_FARCSECH:
        ...
        break;
    default:
        LOGE("error: liquid_firdes_fnyquist(), unknown/unsupported filter type\n");
        exit(1);
    }
...
{
    if (_z <= 0.0f || _z > 1.0f) {
        LOGE("warning: liquid_asechf(), input out of range\n");
        return 0.0f;
    }
