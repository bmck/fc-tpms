                               ...
                               float        _As)
{
    // validate input
    if (_h_len < 2) {
        LOGE("error: firfarrow_%s_create(), filter length must be > 2\n", EXTENSION_FULL);
        exit(1);
    } else if (_p < 1) {
        LOGE("error: firfarrow_%s_create(), polynomial order must be at least 1\n", EXTENSION_FULL);
        exit(1);
    } else if (_fc < 0.0f || _fc > 0.5f) {
        LOGE("error: firfarrow_%s_create(), filter cutoff must be in [0,0.5]\n", EXTENSION_FULL);
        exit(1);
    }
...
{
    LOGI("firfarrow [len : %u, poly-order : %u]\n", _q->h_len, _q->Q);
    LOGI("polynomial coefficients:\n");
    ...
    LOGI("filter coefficients (mu=%8.4f):\n", _q->mu);
    n = _q->h_len;
    for (i=0; i<n; i++) {
        LOGI("  h(%3u) = ", i+1);
                           ...
                           float _mu)
{
    ...
    if (_mu < -1.0f || _mu > 1.0f) {
        LOGE("warning: firfarrow_%s_set_delay(), delay out of range\n", EXTENSION_FULL);
    }
...
#if FIRFARROW_DEBUG
            LOGI("  %3u : x=%12.8f, mu=%12.8f, h0=%12.8f, h1=%12.8f, hp=%12.8f\n",
                    j, x, mu, h0, h1, h0*h1);
#endif
