                                     ...
                                     float _As)
{
    if (_df > 0.5f || _df <= 0.0f) {
        LOGE("error: estimate_req_filter_len(), invalid bandwidth : %f\n", _df);
        exit(1);
    } else if (_As <= 0.0f) {
        LOGE("error: estimate_req_filter_len(), invalid stopband level : %f\n", _As);
        exit(1);
    }
...
#       error "invalid required filter length estimation method"
        ...
        //LOGI("range[%8.2f, %8.2f] As-hat=%8.2fdB, N=%8.2f (target: %3u taps)\n",
...
#       error "invalid required filter length estimation method"
        ...
        //LOGI("range[%8.5f, %8.5f] df-hat=%8.5fdB, N=%8.2f (target: %3u taps)\n",
...
{
    if (_df > 0.5f || _df <= 0.0f) {
        LOGE("error: estimate_req_filter_len_Kaiser(), invalid bandwidth : %f\n", _df);
        exit(1);
    } else if (_As <= 0.0f) {
        LOGE("error: estimate_req_filter_len(), invalid stopband level : %f\n", _As);
        exit(1);
...
{
    if (_df > 0.5f || _df <= 0.0f) {
        LOGE("error: estimate_req_filter_len_Herrmann(), invalid bandwidth : %f\n", _df);
        exit(1);
    } else if (_As <= 0.0f) {
        LOGE("error: estimate_req_filter_len(), invalid stopband level : %f\n", _As);
        exit(1);
                          ...
                          float *_h)
{
    ...
    if (_mu < -0.5f || _mu > 0.5f) {
        LOGE("error: liquid_firdes_kaiser(), _mu (%12.4e) out of range [-0.5,0.5]\n", _mu);
        exit(1);
    } else if (_fc < 0.0f || _fc > 0.5f) {
        LOGE("error: liquid_firdes_kaiser(), cutoff frequency (%12.4e) out of range (0, 0.5)\n", _fc);
        exit(1);
    } else if (_n == 0) {
        LOGE("error: liquid_firdes_kaiser(), filter length must be greater than zero\n");
        exit(1);
    }
                           ...
                           float *             _h)
{
    ...
    case LIQUID_FIRFILT_FARCSECH:
        ...
        break;
    default:
        LOGE("error: liquid_firdes_nyquist(), invalid filter type '%d'\n", _type);
        exit(1);
    }
}
                            ...
                            float *             _h)
{
    ...
    case LIQUID_FIRFILT_RFARCSECH:
        ...
        break;
    default:
        LOGE("error: liquid_firdes_rnyquist(), invalid root-Nyquist filter type '%d'\n", _type);
        exit(1);
    }
}
                           ...
                           unsigned int _nfft)
{
    ...
    if (_fc < 0.0f || _fc > 0.5f) {
        LOGE("error: liquid_filter_energy(), cut-off frequency must be in [0,0.5]\n");
        exit(1);
    } else if (_h_len == 0) {
        LOGE("error: liquid_filter_energy(), filter length must be greater than zero\n");
        exit(1);
    } else if (_nfft == 0) {
        LOGE("error: liquid_filter_energy(), fft size must be greater than zero\n");
        exit(1);
    }
