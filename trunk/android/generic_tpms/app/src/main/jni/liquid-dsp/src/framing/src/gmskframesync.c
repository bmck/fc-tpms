                                      ...
                                      float complex _x)
{
    ...
    if (_q->preamble_counter == _q->preamble_len) {
        LOGE("warning: gmskframesync_execute_rxpn(), p/n buffer already full!\n");
        return;
    }
...
void gmskframesync_decode_header(gmskframesync _q)
{
    ...
    if (_q->header_dec[n+0] != GMSKFRAME_VERSION) {
        LOGE("warning: gmskframesync_decode_header(), invalid framing version\n");
        _q->header_valid = 0;
        return;
    }
    ...
    if (check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE("warning: gmskframesync_decode_header(), decoded CRC exceeds available\n");
        check = LIQUID_CRC_UNKNOWN;
        _q->header_valid = 0;
    }
    if (fec0 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: gmskframesync_decode_header(), decoded FEC (inner) exceeds available\n");
        fec0 = LIQUID_FEC_UNKNOWN;
        _q->header_valid = 0;
    }
    if (fec1 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: gmskframesync_decode_header(), decoded FEC (outer) exceeds available\n");
...
#else
    LOGE("gmskframesync_debug_enable(): compile-time debugging disabled\n");
#endif
}
...
void gmskframesync_debug_disable(gmskframesync _q)
{
    ...
    _q->debug_enabled = 0;
#else
    LOGE("gmskframesync_debug_disable(): compile-time debugging disabled\n");
#endif
}
                               ...
                               const char *  _filename)
{
#if DEBUG_GMSKFRAMESYNC
    if (!_q->debug_objects_created) {
        LOGE("error: gmskframe_debug_print(), debugging objects don't exist; enable debugging first\n");
        return;
    }

    FILE* fid = fopen(_filename,"w");
    if (!fid) {
        LOGE( "error: gmskframesync_debug_print(), could not open '%s' for writing\n", _filename);
        return;
    }
    ...
    LOGI("t = 0:(num_samples-1);\n");
    unsigned int i;
    float complex * rc;

    // write x
    LOGI("x = zeros(1,num_samples);\n");
    ...
    float * r;
    LOGI("fi = zeros(1,num_samples);\n");
    windowf_read(_q->debug_fi, &r);
    for (i=0; i<DEBUG_GMSKFRAMESYNC_BUFFER_LEN; i++)
        LOGI("fi(%4u) = %12.4e;\n", i+1, r[i]);
    LOGI("\n\n");
    LOGI("figure;\n");
    LOGI("plot(1:length(fi),fi);\n");
    LOGI("ylabel('Inst. Freq.');\n");
    LOGI("\n\n");

    // write matched filter output
    LOGI("mf = zeros(1,num_samples);\n");
    windowf_read(_q->debug_mf, &r);
    for (i=0; i<DEBUG_GMSKFRAMESYNC_BUFFER_LEN; i++)
        LOGI("mf(%4u) = %12.4e;\n", i+1, r[i]);
    LOGI("\n\n");
    LOGI("figure;\n");
    LOGI("plot(1:length(mf),mf);\n");
    LOGI("ylabel('MF output');\n");
    LOGI("\n\n");

#if 0
    // write framesyms
    LOGI("framesyms = zeros(1,num_samples);\n");
    windowf_read(_q->debug_framesyms, &r);
    for (i=0; i<DEBUG_GMSKFRAMESYNC_BUFFER_LEN; i++)
        LOGI("framesyms(%4u) = %12.4e;\n", i+1, r[i]);
    LOGI("\n\n");
    LOGI("figure;\n");
    LOGI("plot(t,framesyms,'x')\n");
    LOGI("xlabel('time (symbol index)');\n");
    LOGI("ylabel('GMSK demodulator output');\n");
    LOGI("grid on;\n");
    LOGI("\n\n");
#endif
    ...
    LOGI("gmskframesync/debug: results written to '%s'\n", _filename);
#else
    LOGE("gmskframesync_debug_print(): compile-time debugging disabled\n");
#endif
