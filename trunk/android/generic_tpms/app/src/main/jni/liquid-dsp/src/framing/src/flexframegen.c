...
void flexframegen_print(flexframegen _q)
{
    ...
    LOGI("  tail          : %u symbols\n", _q->m);
    LOGI("  total         : %u symbols\n", num_frame_symbols);
    LOGI("  efficiency    : %.2f bits/second/Hz\n", eta);
}
                          ...
                          flexframegenprops_s * _props)
{
    ...
    if (_q->frame_assembled) {
        LOGE( "warning: flexframegen_setprops(), frame is already assembled; must reset() first\n");
        return -1;
    }
    ...
    if (_props->check == LIQUID_CRC_UNKNOWN || _props->check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE( "error: flexframegen_setprops(), invalid/unsupported CRC scheme\n");
        exit(1);
    } else if (_props->fec0 == LIQUID_FEC_UNKNOWN || _props->fec1 == LIQUID_FEC_UNKNOWN) {
        LOGE( "error: flexframegen_setprops(), invalid/unsupported FEC scheme\n");
        exit(1);
    } else if (_props->mod_scheme == LIQUID_MODEM_UNKNOWN ) {
        LOGE( "error: flexframegen_setprops(), invalid/unsupported modulation scheme\n");
        exit(1);
    }
...
{
    if (!_q->frame_assembled) {
        LOGE("warning: flexframegen_getframelen(), frame not assembled!\n");
        return 0;
    }
    unsigned int num_frame_symbols =
            64 +                    // preamble p/n sequence length
...
void flexframegen_reconfigure(flexframegen _q)
{
    ...
    if (_q->payload_sym == NULL) {
        LOGE("error: flexframegen_reconfigure(), could not re-allocate payload array\n");
        exit(1);
    }
}
...
float complex flexframegen_generate_symbol(flexframegen _q)
{
    ...
    default:
        LOGE("error: flexframegen_generate_symbol(), unknown/unsupported internal state\n");
        exit(1);
    }
