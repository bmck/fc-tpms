                           ...
                           unsigned int _na)
{
    // validate input
    if (_nb == 0) {
        LOGE("error: iirfilt_%s_create(), numerator length cannot be zero\n", EXTENSION_FULL);
        exit(1);
    } else if (_na == 0) {
        LOGE("error: iirfilt_%s_create(), denominator length cannot be zero\n", EXTENSION_FULL);
        exit(1);
    }
                               ...
                               unsigned int _nsos)
{
    // validate input
    if (_nsos == 0) {
        LOGE("error: iirfilt_%s_create_sos(), filter must have at least one 2nd-order section\n", EXTENSION_FULL);
        exit(1);
    }
                               ...
                               float _K)
{
    // validate input
    if (_w <= 0.0f || _w >= 1.0f) {
        LOGE("error: iirfilt_%s_create_pll(), bandwidth must be in (0,1)\n", EXTENSION_FULL);
        exit(1);
    } else if (_zeta <= 0.0f || _zeta >= 1.0f) {
        LOGE("error: iirfilt_%s_create_pll(), damping factor must be in (0,1)\n", EXTENSION_FULL);
        exit(1);
    } else if (_K <= 0.0f) {
        LOGE("error: iirfilt_%s_create_pll(), loop gain must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    }
