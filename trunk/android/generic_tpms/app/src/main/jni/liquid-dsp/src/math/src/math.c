...
{
    if (_x == 0) {
        LOGE("error: liquid_nextpow2(), input must be greater than zero\n");
        exit(1);
    }
...
float liquid_nchoosek(unsigned int _n, unsigned int _k)
{
    //
    if (_k > _n) {
        LOGE("error: liquid_nchoosek(), _k cannot exceed _n\n");
        exit(1);
    } else if (_k == 0 || _k == _n) {
        return 1;
    }
                 ...
                 float _beta)
{
    ...
    // validate input
    if (_n >= _N) {
        LOGE("error: liquid_kbd(), index exceeds maximum\n");
        exit(1);
    } else if (_N == 0) {
        LOGE("error: liquid_kbd(), window length must be greater than zero\n");
        exit(1);
    } else if ( _N % 2 ) {
        LOGE("error: liquid_kbd(), window length must be odd\n");
        exit(1);
    }
                       ...
                       float * _w)
{
    ...
    // validate input
    if (_n == 0) {
        LOGE("error: liquid_kbd_window(), window length must be greater than zero\n");
        exit(1);
    } else if ( _n % 2 ) {
        LOGE("error: liquid_kbd_window(), window length must be odd\n");
        exit(1);
    } else if ( _beta < 0.0f ) {
        LOGE("error: liquid_kbd_window(), _beta must be positive\n");
        exit(1);
    }
             ...
             float _mu)
{
    // validate input
    if (_n > _N) {
        LOGE("error: kaiser(), sample index must not exceed window length\n");
        exit(1);
    } else if (_beta < 0) {
        LOGE("error: kaiser(), beta must be greater than or equal to zero\n");
        exit(1);
    } else if (_mu < -0.5 || _mu > 0.5) {
        LOGE("error: kaiser(), fractional sample offset must be in [-0.5,0.5]\n");
        exit(1);
    }
