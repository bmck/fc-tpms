...
{
    //LOGI("buffer_read() trying to read %u elements (%u available)\n", *_n, _b->num_elements);
#if 0
    if (*_n > _b->num_elements) {
        LOGI("error: buffer_read(), cannot read more elements than are available\n");
        *_v = NULL;
        *_n = 0;
        return;
    } else
...
{
    //LOGI("buffer_s_read() reading %u elements\n", _b->num_elements);
    *_v = _b->v;
    *_n = _b->num_elements;
}
...
void BUFFER(_c_release)(BUFFER() _b, unsigned int _n)
{
    ...
    if (_n > _b->num_elements) {
        LOGI("error: buffer_c_release(), cannot release more elements in buffer than exist\n");
        return;
    }
...
void BUFFER(_c_write)(BUFFER() _b, T * _v, unsigned int _n)
{
    ...
    if (_n > (_b->len - _b->num_elements)) {
        LOGI("error: buffer_write(), cannot write more elements than are available\n");
        return;
    }
...
{
    if (_n > (_b->len - _b->num_elements)) {
        LOGI("error: buffer_s_write(), cannot write more elements than are available\n");
        return;
    }
