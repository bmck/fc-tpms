                          ...
                          float *      _h)
{
    // validate input
    if ( _k < 1 ) {
        LOGE("error: liquid_firdes_gmsktx(): k must be greater than 0\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_gmsktx(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_gmsktx(): beta must be in [0,1]\n");
        exit(1);
    } else;
                          ...
                          float *      _h)
{
    // validate input
    if ( _k < 1 ) {
        LOGE("error: liquid_firdes_gmskrx(): k must be greater than 0\n");
        exit(1);
    } else if ( _m < 1 ) {
        LOGE("error: liquid_firdes_gmskrx(): m must be greater than 0\n");
        exit(1);
    } else if ( (_beta < 0.0f) || (_beta > 1.0f) ) {
        LOGE("error: liquid_firdes_gmskrx(): beta must be in [0,1]\n");
        exit(1);
    } else;
