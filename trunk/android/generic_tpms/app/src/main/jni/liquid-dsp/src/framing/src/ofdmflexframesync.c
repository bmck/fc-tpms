                                           ...
                                           void *             _userdata)
{
    ...
    // validate input
    if (_M < 8) {
        LOGE("warning: ofdmflexframesync_create(), less than 8 subcarriers\n");
    } else if (_M % 2) {
        LOGE("error: ofdmflexframesync_create(), number of subcarriers must be even\n");
        exit(1);
    } else if (_cp_len > _M) {
        LOGE("error: ofdmflexframesync_create(), cyclic prefix length cannot exceed number of subcarriers\n");
        exit(1);
    }
                                        ...
                                        void * _userdata)
{
    ...
    LOGI("******* ofdmflexframesync callback invoked!\n");
#endif
    ...
    case OFDMFLEXFRAMESYNC_STATE_PAYLOAD:
        ...
        break;
    default:
        LOGE("error: ofdmflexframesync_internal_callback(), unknown/unsupported internal state\n");
        exit(1);
    }
...
{
#if DEBUG_OFDMFLEXFRAMESYNC
    LOGI("  ofdmflexframesync extracting header...\n");
#endif
...
#  endif
    ...
    if (_q->header[n+0] != OFDMFLEXFRAME_PROTOCOL) {
        LOGE("warning: ofdmflexframesync_decode_header(), invalid framing version\n");
        _q->header_valid = 0;
    }
    ...
    if (mod_scheme == 0 || mod_scheme >= LIQUID_MODEM_NUM_SCHEMES) {
        LOGE("warning: ofdmflexframesync_decode_header(), invalid modulation scheme\n");
        _q->header_valid = 0;
        return;
    }
    ...
    if (check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE("warning: ofdmflexframesync_decode_header(), decoded CRC exceeds available\n");
        check = LIQUID_CRC_UNKNOWN;
        _q->header_valid = 0;
    }
    if (fec0 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: ofdmflexframesync_decode_header(), decoded FEC (inner) exceeds available\n");
        fec0 = LIQUID_FEC_UNKNOWN;
        _q->header_valid = 0;
    }
    if (fec1 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: ofdmflexframesync_decode_header(), decoded FEC (outer) exceeds available\n");
...
#if DEBUG_OFDMFLEXFRAMESYNC
        LOGI("      * payload mod syms:   %u symbols\n", _q->payload_mod_len);
#endif
    }
}
