                                         ...
                                         ofdmflexframegenprops_s * _fgprops)
{
    // validate input
    if (_M < 2) {
        LOGE("error: ofdmflexframegen_create(), number of subcarriers must be at least 2\n");
        exit(1);
    } else if (_M % 2) {
        LOGE("error: ofdmflexframegen_create(), number of subcarriers must be even\n");
        exit(1);
    }
...
void ofdmflexframegen_print(ofdmflexframegen _q)
{
    ...
    LOGI("    frame assembled     :   %s\n", _q->frame_assembled ? "yes" : "no");
    if (_q->frame_assembled) {
        LOGI("    payload:\n");
        LOGI("      * decoded bytes   :   %-u\n", _q->payload_dec_len);
        LOGI("      * encoded bytes   :   %-u\n", _q->payload_enc_len);
        LOGI("      * modulated syms  :   %-u\n", _q->payload_mod_len);
        LOGI("    total OFDM symbols  :   %-u\n", ofdmflexframegen_getframelen(_q));
        LOGI("      * S0 symbols      :   %-u @ %u\n", 2, _q->M+_q->cp_len);
        LOGI("      * S1 symbols      :   %-u @ %u\n", 1, _q->M+_q->cp_len);
        LOGI("      * header symbols  :   %-u @ %u\n", _q->num_symbols_header,  _q->M+_q->cp_len);
        LOGI("      * payload symbols :   %-u @ %u\n", _q->num_symbols_payload, _q->M+_q->cp_len);
        ...
        LOGI("    spectral efficiency :   %-6.4f b/s/Hz\n", (float)num_bits / (float)num_samples);
    }
}
                               ...
                               ofdmflexframegenprops_s * _props)
{
    ...
    if (_props->check == LIQUID_CRC_UNKNOWN || _props->check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE( "error: ofdmflexframegen_setprops(), invalid/unsupported CRC scheme\n");
        exit(1);
    } else if (_props->fec0 == LIQUID_FEC_UNKNOWN || _props->fec1 == LIQUID_FEC_UNKNOWN) {
        LOGE( "error: ofdmflexframegen_setprops(), invalid/unsupported FEC scheme\n");
        exit(1);
    } else if (_props->mod_scheme == LIQUID_MODEM_UNKNOWN ) {
        LOGE( "error: ofdmflexframegen_setprops(), invalid/unsupported modulation scheme\n");
        exit(1);
    }
                               ...
                               unsigned int     _payload_len)
{
    ...
    LOGI("wrote %u symbols (expected %u)\n", num_written, _q->payload_mod_len);
#endif
}
                                 ...
                                 liquid_float_complex * _buffer)
{
    ...
    if ( !_q->frame_assembled ) {
        LOGE("warning: ofdmflexframegen_writesymbol(), frame not assembled\n");
        return 1;
    }
    ...
    default:
        LOGE("error: ofdmflexframegen_writesymbol(), unknown/unsupported internal state\n");
        exit(1);
    }
