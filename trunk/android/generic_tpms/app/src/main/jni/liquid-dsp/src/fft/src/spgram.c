                         ...
                         unsigned int _window_len)
{
    // validate input
    if (_nfft < 2) {
        LOGE("error: spgram%s_create(), fft size must be at least 2\n", EXTENSION);
        exit(1);
    } else if (_window_len > _nfft) {
        LOGE("error: spgram%s_create(), window size cannot exceed fft size\n", EXTENSION);
        exit(1);
    } else if (_window_len == 0) {
        LOGE("error: spgram%s_create(), window size must be greater than zero\n", EXTENSION);
        exit(1);
    }
                                ...
                                float        _beta)
{
    // validate input
    if (_nfft < 2) {
        LOGE("error: spgram%s_create_kaiser(), fft size must be at least 2\n", EXTENSION);
        exit(1);
    } else if (_window_len > _nfft) {
        LOGE("error: spgram%s_create_kaiser(), window size cannot exceed fft size\n", EXTENSION);
        exit(1);
    } else if (_window_len == 0) {
        LOGE("error: spgram%s_create_kaiser(), window size must be greater than zero\n", EXTENSION);
        exit(1);
    } else if (_beta <= 0.0f) {
        LOGE("error: spgram%s_create_kaiser(), beta must be greater than zero\n", EXTENSION);
        exit(1);
    }
                             ...
                             unsigned int _n)
{
    ...
    if (_alpha < 0.0f || _alpha > 1.0f) {
        LOGE("error: spgram%s_accumulate_psd(), alpha must be in (0,1)\n", EXTENSION);
        exit(1);
    }
                                 ...
                                 T *      _x)
{
    ...
    if (_q->num_transforms == 0)
        LOGE("warning: spgram%s_write_accumulation(), no transforms taken yet\n", EXTENSION);
#endif
                           ...
                           T *          _psd)
{
    ...
    if (_n == 0) {
        LOGE("warning: spgram%s_estimate_psd(), input size is zero\n", EXTENSION);
        return;
    }
