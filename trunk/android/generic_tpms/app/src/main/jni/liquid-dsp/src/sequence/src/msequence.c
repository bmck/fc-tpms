                           ...
                           unsigned int _a)
{
    ...
    if (_m > LIQUID_MAX_MSEQUENCE_M || _m < LIQUID_MIN_MSEQUENCE_M) {
        LOGE("error: msequence_create(), m not in range\n");
        exit(1);
    }
...
msequence msequence_create_genpoly(unsigned int _g)
{
    ...
    // validate input
    if (t < 2) {
        LOGE("error: msequence_create_genpoly(), invalid generator polynomial: 0x%x\n", _g);
        exit(1);
    }
...
void msequence_print(msequence _m)
{
    ...
    LOGI("msequence: m=%u (n=%u):\n", _m->m, _m->n);

    // print shift register
    LOGI("    shift register: ");
    for (i=0; i<_m->m; i++)
        LOGI("%c", ((_m->v) >> (_m->m-i-1)) & 0x01 ? '1' : '0');
    LOGI("\n");

    // print generator polynomial
    LOGI("    generator poly: ");
    for (i=0; i<_m->m; i++)
        LOGI("%c", ((_m->g) >> (_m->m-i-1)) & 0x01 ? '1' : '0');
    LOGI("\n");
}
                              ...
                              msequence _ms)
{
    ...
    if (_ms->n > LIQUID_MAX_MSEQUENCE_LENGTH) {
        LOGE("error: bsequence_init_msequence(), msequence length exceeds maximum\n");
        exit(1);
    }
#endif
