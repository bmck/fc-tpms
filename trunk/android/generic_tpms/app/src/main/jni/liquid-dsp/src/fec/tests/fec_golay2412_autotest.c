...
{
    if (_n > 24) {
        LOGE("error: golay2412_generate_error_vector(), cannot generate more than 24 errors\n");
        exit(1);
    }
...
void autotest_golay2412_codec()
{
    ...
    for (num_errors=0; num_errors<=3; num_errors++) {
        for (i=0; i<num_trials; i++) {
            ...
            LOGI("error index : %u\n", i);
            // print results
            LOGI("    sym org     :   "); liquid_print_bitstring(sym_org, n); LOGI("\n");
            LOGI("    sym enc     :   "); liquid_print_bitstring(sym_enc, k); LOGI("\n");
            LOGI("    sym rec     :   "); liquid_print_bitstring(sym_rec, k); LOGI("\n");
            LOGI("    sym dec     :   "); liquid_print_bitstring(sym_dec, n); LOGI("\n");
