                           ...
                           unsigned int _h_len)
{
    // validate input
    if (_k < 2) {
        LOGE("error: symsync_%s_create(), input sample rate must be at least 2\n", EXTENSION_FULL);
        exit(1);
    } else if (_h_len == 0) {
        LOGE("error: symsync_%s_create(), filter length must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    } else if ( (_h_len-1) % _M ) {
        LOGE("error: symsync_%s_create(), filter length must be of the form: h_len = m*_k*_M + 1 \n", EXTENSION_FULL);
        exit(1);
    } else if (_M == 0) {
        LOGE("error: symsync_%s_create(), number of filter banks must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    }
                                    ...
                                    unsigned int _M)
{
    // validate input
    if (_k < 2) {
        LOGE("error: symsync_%s_create_rnyquist(), samples/symbol must be at least 2\n", EXTENSION_FULL);
        exit(1);
    } else if (_m == 0) {
        LOGE("error: symsync_%s_create_rnyquist(), filter delay (m) must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    } else if (_beta < 0.0f || _beta > 1.0f) {
        LOGE("error: symsync_%s_create_rnyquist(), filter excess bandwidth must be in [0,1]\n", EXTENSION_FULL);
        exit(1);
    }
                                  ...
                                  unsigned int _M)
{
    // validate input
    if (_k < 2) {
        LOGE("error: symsync_%s_create_kaiser(), samples/symbol must be at least 2\n", EXTENSION_FULL);
        exit(1);
    } else if (_m == 0) {
        LOGE("error: symsync_%s_create_kaiser(), filter delay (m) must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    } else if (_beta < 0.0f || _beta > 1.0f) {
        LOGE("error: symsync_%s_create_kaiser(), filter excess bandwidth must be in [0,1]\n", EXTENSION_FULL);
        exit(1);
    }
                               ...
                               unsigned int _k_out)
{
    // validate input
    if (_k_out == 0) {
        LOGE("error: symsync_%s_output_rate(), output rate must be greater than 0\n", EXTENSION_FULL);
        exit(1);
    }
                         ...
                         float     _bt)
{
    ...
    if (_bt < 0.0f || _bt > 1.0f) {
        LOGE("error: symsync_%s_set_lf_bt(), bandwidth must be in [0,1]\n", EXTENSION_FULL);
        exit(1);
    }
...
#if DEBUG_SYMSYNC_PRINT
    LOGI("q : %12.8f, rate : %12.8f, del : %12.8f, q_hat : %12.8f\n", _q->q, _q->rate, _q->del, _q->q_hat);
#endif
}
                                 ...
                                 const char * _filename)
{
    ...
    if (!fid) {
        LOGE("error: symsync_%s_output_debug_file(), could not open '%s' for writing\n", EXTENSION_FULL, _filename);
        return;
    }
    ...
    for (i=0; i<_q->h_len; i++) {
        ...
        for (n=0; n<_q->npfb; n++) {
            ...
            LOGI("h(%4u) = %12.8f; dh(%4u) = %12.8f;\n", i*_q->npfb+n+1, crealf(mf), i*_q->npfb+n+1, crealf(dmf));
        }
    }
    // plot response
    LOGI("\n");
    LOGI("figure;\n");
    LOGI("th = [0:(h_len*npfb-1)]/(k*npfb) - h_len/(2*k);\n");
    LOGI("m  = abs(round(th(1)));\n");
    //LOGI("plot(t,h,t,dh);\n");
    LOGI("subplot(3,1,1),\n");
    LOGI("  plot(th, h, 'LineWidth', 2, 'Color', [0 0.5 0.2]);\n");
    LOGI("  ylabel('MF');\n");
    LOGI("  axis([-m m -0.25 1.25]);\n");
    LOGI("  grid on;\n");
    LOGI("subplot(3,1,2),\n");
    LOGI("  plot(th, dh, 'LineWidth', 2, 'Color', [0 0.2 0.5]);\n");
    LOGI("  ylabel('dMF');\n");
    LOGI("  axis([-m m -0.10 0.10]);\n");
    LOGI("  grid on;\n");
    LOGI("subplot(3,1,3),\n");
    LOGI("  plot(th,-h.*dh, 'LineWidth', 2, 'Color', [0.5 0 0]);\n");
    LOGI("  xlabel('Symbol Index');\n");
    LOGI("  ylabel('-MF*dMF');\n");
    LOGI("  axis([-m m -0.08 0.08]);\n");
    LOGI("  grid on;\n");
    ...
    LOGI("subplot(2,1,1);\n");
    LOGI("  hold on;\n");
    LOGI("  plot(t,b,'Color',[0.5 0.5 0.5]);\n");
    LOGI("  plot(t,bf,'LineWidth',2,'Color',[0 0.25 0.5]);\n");
    LOGI("  hold off;\n");
    LOGI("  grid on;\n");
    LOGI("  axis([t(1) t(end) -1 npfb]);\n");
    LOGI("  legend('b','b (soft)',0);\n");
    LOGI("  xlabel('Symbol Index')\n");
    LOGI("  ylabel('Polyphase Filter Index')\n");
    LOGI("subplot(2,1,2);\n");
    LOGI("  hold on;\n");
    LOGI("  plot(t,rate,'LineWidth',2,'Color',[0 0.25 0.5]);\n");
    LOGI("  hold off;\n");
    LOGI("  grid on;\n");
    LOGI("  axis([t(1) t(end) 0.99 1.01]);\n");
    LOGI("  xlabel('Symbol Index')\n");
    LOGI("  ylabel('Rate')\n");
    LOGI("%% done.\n");
#endif
    ...
    fclose(fid);
    LOGI("symsync: internal results written to '%s'\n", _filename);
}
