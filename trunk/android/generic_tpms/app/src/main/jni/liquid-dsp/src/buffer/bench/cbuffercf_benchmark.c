                     ...
                     unsigned int        _read_size)
{
    // validate input
    if (_n < 2) {
        LOGE("error: cbuffercf_bench(), number of elements must be at least 2\n");
        exit(1);
    } else if (_write_size > _n-1) {
        LOGE("error: cbuffercf_bench(), write size must be in (0,n)\n");
        exit(1);
    } else if (_read_size > _n-1) {
        LOGE("error: cbuffercf_bench(), read size must be in (0,n)\n");
        exit(1);
    }
