...
void bsequence_print(bsequence _bs)
{
    ...
    LOGI("bsequence[%6u]:     ", _bs->num_bits);
    for (i=0; i<_bs->s_len; i++) {
        ...
        for (j=0; j<p; j++) {
            if (i==0 && j<p-_bs->num_bits_msb)
                LOGI(".");    // print '.' for each bit in byte not included in first byte
            else
                LOGI("%c", (chunk >> (p-j-1)) & 0x01 ? '1' : '0');
                               ...
                               bsequence _bs2)
{
    ...
    if ( _bs1->s_len != _bs2->s_len ) {
        LOGI("error: bsequence_correlate(), binary sequences must be the same length!\n");
        exit(-1);
    }
                   ...
                   bsequence _bs3)
{
    ...
    {
        LOGE("error: bsequence_add(), binary sequences must be same length!\n");
        exit(-1);
    }
                   ...
                   bsequence _bs3)
{
    ...
    if ( _bs1->s_len != _bs2->s_len ||
         ...
         _bs2->s_len != _bs3->s_len )
    {
        LOGE("error: bsequence_mul(), binary sequences must be same length!\n");
        exit(-1);
    }
...
{
    if (_i >= _bs->num_bits) {
        LOGE("error: bsequence_index(), invalid index %u\n", _i);
        exit(-1);
    }
...
void bsequence_create_ccodes(bsequence _qa, bsequence _qb)
{
    ...
    if (_qa->num_bits != _qb->num_bits) {
        LOGI("error: bsequence_create_ccodes(), sequence lengths must match\n");
        exit(1);
    } else if (_qa->num_bits < 8) {
        LOGI("error: bsequence_create_ccodes(), sequence too short\n");
        exit(1);
    } else if ( (_qa->num_bits)%8 != 0 ) {
        LOGI("error: bsequence_create_ccodes(), sequence must be multiple of 8\n");
        exit(1);
    }
