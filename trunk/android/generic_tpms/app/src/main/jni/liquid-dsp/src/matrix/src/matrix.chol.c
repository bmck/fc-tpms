                   ...
                   T *          _L)
{
    ...
    for (j=0; j<_n; j++) {
        ...
        if ( creal(A_jj) < 0.0 ) {
            LOGE("warning: matrix_chol(), matrix is not positive definite (real{A[%u,%u]} = %12.4e < 0)\n",j,j,creal(A_jj));
            return;
        }
#if T_COMPLEX
        if ( fabs(cimag(A_jj)) > 0.0 ) {
            LOGE("warning: matrix_chol(), matrix is not positive definite (|imag{A[%u,%u]}| = %12.4e > 0)\n",j,j,fabs(cimag(A_jj)));
            return;
        }
#endif
        ...
        if ( creal(A_jj) < t0 ) {
            LOGE("warning: matrix_chol(), matrix is not positive definite (real{A[%u,%u]} = %12.4e < %12.4e)\n",j,j,creal(A_jj),t0);
            return;
        }
        L_jj = sqrt( A_jj - t0 );
