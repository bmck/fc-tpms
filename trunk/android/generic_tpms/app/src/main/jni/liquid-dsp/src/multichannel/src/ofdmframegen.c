                                 ...
                                 unsigned char * _p)
{
    // validate input
    if (_M < 2) {
        LOGE("error: ofdmframegen_create(), number of subcarriers must be at least 2\n");
        exit(1);
    } else if (_M % 2) {
        LOGE("error: ofdmframegen_create(), number of subcarriers must be even\n");
        exit(1);
    } else if (_cp_len > _M) {
        LOGE("error: ofdmframegen_create(), cyclic prefix cannot exceed symbol length\n");
        exit(1);
    } else if (_taper_len > _cp_len) {
        LOGE("error: ofdmframegen_create(), taper length cannot exceed cyclic prefix\n");
        exit(1);
    }
    ...
    if ( (q->M_pilot + q->M_data) == 0) {
        LOGE("error: ofdmframegen_create(), must have at least one enabled subcarrier\n");
        exit(1);
    } else if (q->M_data == 0) {
        LOGE("error: ofdmframegen_create(), must have at least one data subcarriers\n");
        exit(1);
    } else if (q->M_pilot < 2) {
        LOGE("error: ofdmframegen_create(), must have at least two pilot subcarriers\n");
        exit(1);
    }
