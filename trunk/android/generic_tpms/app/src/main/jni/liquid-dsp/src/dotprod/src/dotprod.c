                    ...
                    TO *         _y)
{
    ...
    for (i=0; i<t; i+=4) {
        r += _h[i]   * _x[i];
//        LOGI("i=%d, _h[%d]=%f+%fj, _x[%d]=%f+%fj, r = %f+%fj\n", i, i, creal(_h[i]), cimag(_h[i]), i, creal(_x[i]), cimag(_x[i]), creal(r), cimag(r));
        r += _h[i+1] * _x[i+1];
//        LOGI("i=%d, _h[%d]=%f+%fj, _x[%d]=%f+%fj, r = %f+%fj\n", i+1, i+1, creal(_h[i+1]), cimag(_h[i+1]), i+1, creal(_x[i+1]), cimag(_x[i+1]), creal(r), cimag(r));
        r += _h[i+2] * _x[i+2];
//        LOGI("i=%d, _h[%d]=%f+%fj, _x[%d]=%f+%fj, r = %f+%fj\n", i+2, i+2, creal(_h[i+2]), cimag(_h[i+2]), i+2, creal(_x[i+2]), cimag(_x[i+2]), creal(r), cimag(r));
        r += _h[i+3] * _x[i+3];
//        LOGI("i=%d, _h[%d]=%f+%fj, _x[%d]=%f+%fj, r = %f+%fj\n", i+3, i+3, creal(_h[i+3]), cimag(_h[i+3]), i+3, creal(_x[i+3]), cimag(_x[i+3]), creal(r), cimag(r));
    }
...
//        LOGI("i=%d, _h[%d]=%f+%fj, _x[%d]=%f+%fj, r = %f+%fj\n", i, i, creal(_h[i]), cimag(_h[i]), i, creal(_x[i]), cimag(_x[i]), creal(r), cimag(r));
    ...
    // return result
//    LOGI("(%s:%d) r = %f+%fj, _y=%p\n", __FILE__, __LINE__, creal(r), cimag(r), _y);
    *_y = r;
}
...
{
    LOGI("dotprod [portable, %u coefficients]:\n", _q->n);
    unsigned int i;
    for (i=0; i<_q->n; i++) {
        LOGI("  %4u: %12.8f + j*%12.8f\n", i,
                                             crealf(_q->h[i]),
                                             cimagf(_q->h[i]));
    }
}
...
//    LOGI("(%s:%d) _y=%p, _x=%p, _q=%p\n", __FILE__, __LINE__, _y, _x, _q);
    DOTPROD(_run4)(_q->h, _x, _q->n, _y);
//    LOGI("(%s:%d) here, _y=%p\n", __FILE__, __LINE__, _y);
}
