                               ...
                               TC *         _h)
{
    ...
    if (_type != LIQUID_ANALYZER && _type != LIQUID_SYNTHESIZER) {
        LOGE("error: firpfbch2_%s_create(), invalid type %d\n", EXTENSION_FULL, _type);
        exit(1);
    } else if (_M < 2 || _M % 2) {
        LOGE("error: firpfbch2_%s_create(), number of channels must be greater than 2 and even\n", EXTENSION_FULL);
        exit(1);
    } else if (_m < 1) {
        LOGE("error: firpfbch2_%s_create(), filter semi-length must be at least 1\n", EXTENSION_FULL);
        exit(1);
    }
                                      ...
                                      float        _As)
{
    // validate input
    if (_type != LIQUID_ANALYZER && _type != LIQUID_SYNTHESIZER) {
        LOGE("error: firpfbch2_%s_create_kaiser(), invalid type %d\n", EXTENSION_FULL, _type);
        exit(1);
    } else if (_M < 2 || _M % 2) {
        LOGE("error: firpfbch2_%s_create_kaiser(), number of channels must be greater than 2 and even\n", EXTENSION_FULL);
        exit(1);
    } else if (_m < 1) {
        LOGE("error: firpfbch2_%s_create_kaiser(), filter semi-length must be at least 1\n", EXTENSION_FULL);
        exit(1);
    }
                         ...
                         TO *        _y)
{
    ...
    case LIQUID_SYNTHESIZER:
        ...
        return;
    default:
        LOGE("error: firpfbch2_%s_execute(), invalid type\n", EXTENSION_FULL);
        exit(1);
    }
}
