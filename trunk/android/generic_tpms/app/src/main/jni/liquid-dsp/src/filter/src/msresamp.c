                             ...
                             float _As)
{
    // validate input
    if (_r <= 0.0f) {
        LOGE("error: msresamp_%s_create(), resampling rate must be greater than zero\n", EXTENSION_FULL);
        exit(1);
    }
...
{
    LOGI("multi-stage resampler\n");
    LOGI("    composite rate      : %12.10f\n", _q->rate);
    LOGI("    type                : %s\n", _q->type == LIQUID_RESAMP_INTERP ? "interp" : "decim");
    LOGI("    num halfband stages : %u\n", _q->num_halfband_stages);
    LOGI("    halfband rate       : %s%u\n", _q->type == LIQUID_RESAMP_INTERP ? "" : "1/",
                                                 1<<_q->num_halfband_stages);
    LOGI("    arbitrary rate      : %12.10f\n", _q->rate_arbitrary);
    LOGI("    stages:\n");
    ...
    if (_q->type == LIQUID_RESAMP_INTERP) {
        ...
        r *= rate;  // update rate
        LOGI("    [%2u, r=%11.7f] : arbitrary, r=%12.8f\n", stage, r, rate);
        stage++;
    }
    ...
    for (i=0; i<_q->num_halfband_stages; i++) {
        ...
        r *= rate;
        LOGI("    [%2u, r=%11.7f] : halfband,  r=%5.1f\n", stage, r, rate);
        stage++;
    }
