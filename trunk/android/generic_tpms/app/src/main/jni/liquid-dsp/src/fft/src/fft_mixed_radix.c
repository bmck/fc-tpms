                                        ...
                                        int          _flags)
{
    ...
    if (Q==0) {
        LOGE("error: fft_create_plan_mixed_radix(), _nfft=%u is prime\n", _nfft);
        exit(1);
    } else if ( (_nfft % Q) != 0 ) {
        LOGE("error: fft_create_plan_mixed_radix(), _nfft=%u is not divisible by Q=%u\n", _nfft, Q);
        exit(1);
    }
...
void FFT(_execute_mixed_radix)(FFT(plan) _q)
{
    ...
    LOGI("computing %u DFTs of size %u\n", Q, P);
#endif
    for (i=0; i<Q; i++) {
...
unsigned int FFT(_estimate_mixed_radix)(unsigned int _nfft)
{
    ...
    if (num_factors < 2) {
        LOGE("warning: fft_estimate_mixed_radix(), %u is prime\n", _nfft);
        return 0;
    }
