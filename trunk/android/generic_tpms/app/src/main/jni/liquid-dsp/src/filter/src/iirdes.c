                     ...
                     float complex * _p)
{
    // validate input
    if (_tol < 0) {
        LOGE("error: liquid_cplxpair(), tolerance must be positive\n");
        exit(1);
    }
    ...
    for (i=0; i<_n; i++) {
        ...
        if (cimagf(_z[i]) > _tol) {
            LOGE("warning, liquid_cplxpair(), complex numbers cannot be paired\n");
        } else {
            _p[k++] = _z[i];
            paired[i] = 1;
        }
                   ...
                   float complex * _kd)
{
    ...
    *_kd = G;
    ...
    LOGI("zpk_a2df() poles (discrete):\n");
    for (i=0; i<n; i++)
        LOGI("  pd[%3u] = %12.8f + j*%12.8f\n", i, crealf(_pd[i]), cimagf(_pd[i]));
    LOGI("zpk_a2df() zeros (discrete):\n");
    for (i=0; i<n; i++)
        LOGI("  zd[%3u] = %12.8f + j*%12.8f\n", i, crealf(_zd[i]), cimagf(_zd[i]));
    LOGI("zpk_a2df() gain (discrete):\n");
    LOGI("  kd      = %12.8f + j*%12.8f\n", crealf(G), cimagf(G));
#endif
}
                      ...
                      float * _A)
{
    ...
    LOGI("zeros :\n");
    for (i=0; i<_n; i++)
        LOGI("  z[%3u] = %12.8f + j*%12.8f\n", i, crealf(_zd[i]), cimagf(_zd[i]));

    LOGI("poles (conjugate pairs):\n");
    for (i=0; i<_n; i++)
        LOGI("  p[%3u] = %12.8f + j*%12.8f\n", i, crealf(pp[i]), cimagf(pp[i]));
    LOGI("zeros (conjugate pairs):\n");
    for (i=0; i<_n; i++)
                   ...
                   float * _A)
{
    // validate input
    if (_fc <= 0 || _fc >= 0.5) {
        LOGE("error: liquid_iirdes(), cutoff frequency out of range\n");
        exit(1);
    } else if (_f0 < 0 || _f0 > 0.5) {
        LOGE("error: liquid_iirdes(), center frequency out of range\n");
        exit(1);
    } else if (_Ap <= 0) {
        LOGE("error: liquid_iirdes(), pass-band ripple out of range\n");
        exit(1);
    } else if (_As <= 0) {
        LOGE("error: liquid_iirdes(), stop-band ripple out of range\n");
        exit(1);
    } else if (_n == 0) {
        LOGE("error: liquid_iirdes(), filter order must be > 0\n");
        exit(1);
    }
    ...
    case LIQUID_IIRDES_BESSEL:
        ...
        break;
    default:
        LOGE("error: liquid_iirdes(), unknown filter type\n");
        exit(1);
    }
    ...
    LOGI("poles (analog):\n");
    for (i=0; i<npa; i++)
        LOGI("  pa[%3u] = %12.8f + j*%12.8f\n", i, crealf(pa[i]), cimagf(pa[i]));
    LOGI("zeros (analog):\n");
    for (i=0; i<nza; i++)
        LOGI("  za[%3u] = %12.8f + j*%12.8f\n", i, crealf(za[i]), cimagf(za[i]));
    LOGI("gain (analog):\n");
    LOGI("  ka : %12.8f + j*%12.8f\n", crealf(ka), cimagf(ka));
#endif
...
#if LIQUID_IIRDES_DEBUG_PRINT
    LOGI("zeros (digital, low-pass prototype):\n");
    for (i=0; i<_n; i++)
        LOGI("  zd[%3u] = %12.4e + j*%12.4e;\n", i, crealf(zd[i]), cimagf(zd[i]));
    LOGI("poles (digital, low-pass prototype):\n");
    for (i=0; i<_n; i++)
        LOGI("  pd[%3u] = %12.4e + j*%12.4e;\n", i, crealf(pd[i]), cimagf(pd[i]));
    LOGI("gain (digital):\n");
    LOGI("  kd : %12.8f + j*%12.8f\n", crealf(kd), cimagf(kd));
#endif
...
#if LIQUID_IIRDES_DEBUG_PRINT
        // print coefficients
        for (i=0; i<=_n; i++) LOGI("b[%3u] = %12.8f;\n", i, _B[i]);
        for (i=0; i<=_n; i++) LOGI("a[%3u] = %12.8f;\n", i, _A[i]);
#endif
    } else {
                    ...
                    unsigned int _n)
{
    // validate input
    if (_n < 2) {
        LOGE("error: iirdes_isstable(), filter order too low\n");
        exit(1);
    }
    unsigned int i;
