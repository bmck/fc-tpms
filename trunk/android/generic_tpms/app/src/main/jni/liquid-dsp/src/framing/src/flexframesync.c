                           ...
                           unsigned int    _n)
{
    ...
    for (i=0; i<_n; i++) {
        ...
        case FLEXFRAMESYNC_STATE_RXPAYLOAD:
            ...
            break;
        default:
            LOGE("error: flexframesync_exeucte(), unknown/unsupported state\n");
            exit(1);
        }
    }
}
                                  ...
                                  float complex _x)
{
    ...
    if (v != NULL) {
        ...
        LOGI("***** frame detected! tau-hat:%8.4f, dphi-hat:%8.4f, gamma:%8.2f dB\n",
...
void flexframesync_decode_header(flexframesync _q)
{
    ...
    //LOGI("residual offset: dphi=%12.8f, phi=%12.8f\n", dphi_hat, phi_hat);
    ...
    if (protocol != FLEXFRAME_PROTOCOL) {
        LOGE("warning: flexframesync_decode_header(), invalid framing protocol %u (expected %u)\n", protocol, FLEXFRAME_PROTOCOL);
        _q->header_valid = 0;
        return;
    }
    ...
    if (mod_scheme == 0 || mod_scheme >= LIQUID_MODEM_NUM_SCHEMES) {
        LOGE("warning: flexframesync_decode_header(), invalid modulation scheme\n");
        _q->header_valid = 0;
        return;
    } else if (check == LIQUID_CRC_UNKNOWN || check >= LIQUID_CRC_NUM_SCHEMES) {
        LOGE("warning: flexframesync_decode_header(), decoded CRC exceeds available\n");
        _q->header_valid = 0;
        return;
    } else if (fec0 == LIQUID_FEC_UNKNOWN || fec0 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: flexframesync_decode_header(), decoded FEC (inner) exceeds available\n");
        _q->header_valid = 0;
        return;
    } else if (fec1 == LIQUID_FEC_UNKNOWN || fec1 >= LIQUID_FEC_NUM_SCHEMES) {
        LOGE("warning: flexframesync_decode_header(), decoded FEC (outer) exceeds available\n");
        _q->header_valid = 0;
        return;
    }
    ...
    if (_q->payload_sym == NULL || _q->payload_dec == NULL) {
        LOGE("error: flexframesync_decode_header(), could not re-allocate payload arrays\n");
        _q->header_valid = 0;
        return;
    }
...
#if DEBUG_FLEXFRAMESYNC_PRINT
    // print results
    LOGI("flexframesync_decode_header():\n");
...
#else
    LOGE("flexframesync_debug_enable(): compile-time debugging disabled\n");
#endif
}
                               ...
                               const char *  _filename)
{
#if DEBUG_FLEXFRAMESYNC
    if (!_q->debug_objects_created) {
        LOGE("error: flexframesync_debug_print(), debugging objects don't exist; enable debugging first\n");
        return;
    }
    unsigned int i;
    float complex * rc;
    FILE* fid = fopen(_filename,"w");
    LOGI("%% %s: auto-generated file", _filename);
    LOGI("\n\n");
    LOGI("clear all;\n");
    LOGI("close all;\n\n");
    LOGI("n = %u;\n", DEBUG_BUFFER_LEN);
    ...
    // main figure
    LOGI("figure('Color','white','position',[100 100 800 600]);\n");
    ...
    LOGI("\n\n");
    LOGI("subplot(3,2,1:2);\n");
    LOGI("plot(1:length(x),real(x), 1:length(x),imag(x));\n");
    LOGI("grid on;\n");
    LOGI("xlabel('sample index');\n");
    LOGI("ylabel('received signal, x');\n");

    // write p/n sequence
    LOGI("preamble_pn = zeros(1,64);\n");
    rc = _q->preamble_pn;
    for (i=0; i<64; i++)
        LOGI("preamble_pn(%4u) = %12.4e + 1i*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    // write p/n symbols
    LOGI("preamble_rx = zeros(1,64);\n");
    rc = _q->preamble_rx;
    for (i=0; i<64; i++)
        LOGI("preamble_rx(%4u) = %12.4e + 1i*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    // write recovered header symbols (after qpilotsync)
    LOGI("header_mod = zeros(1,%u);\n", _q->header_mod_len);
    rc = _q->header_mod;
    for (i=0; i<_q->header_mod_len; i++)
        LOGI("header_mod(%4u) = %12.4e + j*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    // write raw payload symbols
    LOGI("payload_sym = zeros(1,%u);\n", _q->payload_sym_len);
    rc = _q->payload_sym;
    for (i=0; i<_q->payload_sym_len; i++)
        LOGI("payload_sym(%4u) = %12.4e + j*%12.4e;\n", i+1, crealf(rc[i]), cimagf(rc[i]));

    LOGI("subplot(3,2,[3 5]);\n");
    LOGI("plot(real(header_mod),imag(header_mod),'o');\n");
    LOGI("xlabel('in-phase');\n");
    LOGI("ylabel('quadrature phase');\n");
    LOGI("grid on;\n");
    LOGI("axis([-1 1 -1 1]*1.5);\n");
    LOGI("axis square;\n");
    LOGI("title('Received Header Symbols');\n");

    LOGI("subplot(3,2,[4 6]);\n");
    LOGI("plot(real(payload_sym),imag(payload_sym),'+');\n");
    LOGI("xlabel('in-phase');\n");
    LOGI("ylabel('quadrature phase');\n");
    LOGI("grid on;\n");
    LOGI("axis([-1 1 -1 1]*1.5);\n");
    LOGI("axis square;\n");
    LOGI("title('Received Payload Symbols');\n");

    LOGI("\n\n");
    fclose(fid);

    LOGI("flexframesync/debug: results written to %s\n", _filename);
#else
    LOGE("flexframesync_debug_print(): compile-time debugging disabled\n");
#endif
}
