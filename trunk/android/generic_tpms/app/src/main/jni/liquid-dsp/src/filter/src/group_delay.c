                      ...
                      float _fc)
{
    // validate input
    if (_n == 0) {
        LOGE("error: fir_group_delay(), length must be greater than zero\n");
        exit(1);
    } else if (_fc < -0.5 || _fc > 0.5) {
        LOGE("error: fir_group_delay(), _fc must be in [-0.5,0.5]\n");
        exit(1);
    }
                      ...
                      float _fc)
{
    // validate input
    if (_nb == 0) {
        LOGE("error: iir_group_delay(), numerator length must be greater than zero\n");
        exit(1);
    } else if (_na == 0) {
        LOGE("error: iir_group_delay(), denominator length must be greater than zero\n");
        exit(1);
    } else if (_fc < -0.5 || _fc > 0.5) {
        LOGE("error: iir_group_delay(), _fc must be in [-0.5,0.5]\n");
        exit(1);
    }
