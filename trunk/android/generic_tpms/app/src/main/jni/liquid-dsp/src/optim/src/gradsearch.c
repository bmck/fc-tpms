...
{
    //LOGI("gradient search:\n");
    LOGI("u=%12.4e ",   _q->u);       // utility
#if 0
    // enable more verbose output
    LOGI("|p|=%7.1e ",  _q->pnorm);   // norm(p)
    LOGI("del=%7.1e ",  _q->delta);   // delta
#endif
    LOGI("step=%7.1e ", _q->alpha);   // alpha (step size)
...
float gradsearch_step(gradsearch _q)
{
    ...
    if (i == n) {
        LOGE("warning: gradsearch_step(), function ill-conditioned\n");
                            ...
                            float            _alpha)
{
    ...
    while (continue_running) {
        ...
        //LOGI("  linesearch %6u : alpha=%12.6f, u0=%12.8f, uls=%12.8f\n", num_iterations, alpha, u0, uls);
