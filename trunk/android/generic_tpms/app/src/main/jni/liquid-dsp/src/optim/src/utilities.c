...
{
    if (_n == 0) {
        LOGE("error: liquid_rosenbrock(), input vector length cannot be zero\n");
        exit(1);
    } else if (_n == 1) {
...
{
    if (_n == 0) {
        LOGE("error: liquid_invgauss(), input vector length cannot be zero\n");
        exit(1);
    }
...
{
    if (_n == 0) {
        LOGE("error: liquid_multimodal(), input vector length cannot be zero\n");
        exit(1);
    }
