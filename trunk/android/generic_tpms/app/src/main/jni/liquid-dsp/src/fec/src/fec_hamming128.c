...
unsigned int fec_hamming128_encode_symbol(unsigned int _sym_dec)
{
    // validate input
    if (_sym_dec >= (1<<8)) {
        LOGE("error, fec_hamming128_encode(), input symbol too large\n");
        exit(1);
    }
    ...
    LOGI("parity bits (p1,p2,p4,p8) : (%1u,%1u,%1u,%1u)\n", p1, p2, p4, p8);
#endif
...
unsigned int fec_hamming128_decode_symbol(unsigned int _sym_enc)
{
    // validate input
    if (_sym_enc >= (1<<12)) {
        LOGE("error, fec_hamming128_decode(), input symbol too large\n");
        exit(1);
    }
...
#if DEBUG_FEC_HAMMING128
    LOGI("syndrome bits (s1,s2,s4,s8) : (%1u,%1u,%1u,%1u)\n", s1, s2, s4, s8);
    LOGI("syndrome z : %u\n", z);
#endif
    ...
    if (z) {
        if (z > 12) {
            ...
            //LOGE("warning, fec_hamming128_decode_symbol(), syndrome index exceeds 12\n");
        } else {
            //LOGI("error detected!\n");
            _sym_enc ^= 1 << (12-z);
        }
                                ...
                                unsigned char *_msg_dec)
{
    ...
    for (i=0; i<_dec_msg_len; i++) {
        ...
        //LOGI("  %3u : 0x%.2x > 0x%.2x,  0x%.2x > 0x%.2x (k=%u)\n", i, r0, s0, r1, s1, k);
    }
