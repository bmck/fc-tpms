...
void gmskframegen_print(gmskframegen _q)
{
    // plot
    LOGI("gmskframegen:\n");
    LOGI("  physical properties\n");
    LOGI("    samples/symbol  :   %u\n", _q->k);
    LOGI("    filter delay    :   %u symbols\n", _q->m);
    LOGI("    bandwidth-time  :   %-8.3f\n", _q->BT);
    LOGI("  framing properties\n");
    LOGI("    preamble        :   %-4u symbols\n", _q->preamble_len);
    LOGI("    header          :   %-4u symbols\n", _q->header_len);
    LOGI("    payload         :   %-4u symbols\n", _q->payload_len);
    LOGI("    tail            :   %-4u symbols\n", _q->tail_len);
    LOGI("  packet properties\n");
    LOGI("    crc             :   %s\n", crc_scheme_str[_q->check][1]);
    LOGI("    fec (inner)     :   %s\n", fec_scheme_str[_q->fec0][1]);
    LOGI("    fec (outer)     :   %s\n", fec_scheme_str[_q->fec1][1]);
    LOGI("  total samples     :   %-4u sampels\n", gmskframegen_getframelen(_q));
}
...
unsigned int gmskframegen_getframelen(gmskframegen _q)
{
    if (!_q->frame_assembled) {
        LOGE("warning: gmskframegen_getframelen(), frame not assembled!\n");
        return 0;
    }
                               ...
                               float complex * _y)
{
    ...
    default:
        LOGE("error: gmskframegen_writesymbol(), unknown/unsupported internal state\n");
        exit(1);
    }
