                                ...
                                float        _dt)
{
    // validate input
    if (_k < 2) {
        LOGE("error: eqlms_%s_create_rnyquist(), samples/symbol must be greater than 1\n", EXTENSION_FULL);
        exit(1);
    } else if (_m == 0) {
        LOGE("error: eqlms_%s_create_rnyquist(), filter delay must be greater than 0\n", EXTENSION_FULL);
        exit(1);
    } else if (_beta < 0.0f || _beta > 1.0f) {
        LOGE("error: eqlms_%s_create_rnyquist(), filter excess bandwidth factor must be in [0,1]\n", EXTENSION_FULL);
        exit(1);
    } else if (_dt < -1.0f || _dt > 1.0f) {
        LOGE("error: eqlms_%s_create_rnyquist(), filter fractional sample delay must be in [-1,1]\n", EXTENSION_FULL);
        exit(1);
    }
                               ...
                               float        _fc)
{
    // validate input
    if (_h_len == 0) {
        LOGE("error: eqlms_%s_create_lowpass(), filter length must be greater than 0\n", EXTENSION_FULL);
        exit(1);
    } else if (_fc <= 0.0f || _fc > 0.5f) {
        LOGE("error: eqlms_%s_create_rnyquist(), filter cutoff must be in (0,0.5]\n", EXTENSION_FULL);
        exit(1);
    }
...
{
    if (_mu < 0.0f) {
        LOGE("error: eqlms_%s_set_bw(), learning rate cannot be less than zero\n", EXTENSION_FULL);
        exit(1);
    }
                   ...
                   unsigned int _n)
{
    ...
    if (_n < _q->h_len) {
        LOGE("warning: eqlms_%s_train(), traning sequence less than filter order\n", EXTENSION_FULL);
    }
