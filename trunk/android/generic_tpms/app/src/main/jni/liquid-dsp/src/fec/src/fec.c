...
fec_scheme liquid_getopt_str2fec(const char * _str)
{
    ...
    LOGE("warning: liquid_getopt_str2fec(), unknown/unsupported fec scheme : %s\n", _str);
                                    ...
                                    unsigned int _msg_len)
{
    ...
    case LIQUID_FEC_CONV_V29P67:
    case LIQUID_FEC_CONV_V29P78:
        LOGE( "error: fec_get_enc_msg_length(), convolutional codes unavailable (install libfec)\n");
        exit(-1);

    case LIQUID_FEC_RS_M8:
        LOGE( "error: fec_get_enc_msg_length(), Reed-Solomon codes unavailable (install libfec)\n");
        exit(-1);
#endif
    default:
        LOGI("error: fec_get_enc_msg_length(), unknown/unsupported scheme: %d\n", _scheme);
        exit(-1);
    }
                                       ...
                                       unsigned int _k)
{
    // validate input
    if (_m == 0) {
        LOGE("fec_block_get_enc_msg_len(), input block size cannot be zero\n");
        exit(1);
    } else if (_k < _m) {
        LOGE("fec_block_get_enc_msg_len(), output block size cannot be smaller than input\n");
        exit(1);
    }
...
#if 0
    LOGI("fec_block_get_enc_msg_len(%u,%u,%u)\n", _dec_msg_len, _m, _k);
    LOGI("    dec msg len :   %u bytes\n", _dec_msg_len);
    LOGI("    m           :   %u bits\n", _m);
    LOGI("    k           :   %u bits\n", _k);
    LOGI("    num bits in :   %u bits\n", num_bits_in);
    LOGI("    num blocks  :   %u\n", num_blocks);
    LOGI("    num bits out:   %u bits\n", num_bits_out);
    LOGI("    enc msg len :   %u bytes\n", num_bytes_out);
#endif
                                      ...
                                      unsigned int _p)
{
    ...
    LOGI("msg len :       %3u\n", _dec_msg_len);
    LOGI("num bits in :   %3u\n", num_bits_in);
    LOGI("n (constraint): %3u\n", n);
    LOGI("num bits out:   %3u", num_bits_out);
    LOGI(" = n+(n+p-1)/p = %u+(%u+%u-1)/%u\n", n,n,_p,_p);
    LOGI("num bytes out:  %3u\n", num_bytes_out);
#endif
    return num_bytes_out;
}
                                    ...
                                    unsigned int _kk)
{
    // validate input
    if (_dec_msg_len == 0) {
        LOGE("error: fec_rs_get_enc_msg_len(), _dec_msg_len must be greater than 0\n");
        exit(1);
    }
    ...
    LOGI("dec_msg_len     :   %u\n", _dec_msg_len);
    LOGI("num_blocks      :   %u\n",  num_blocks);
    LOGI("dec_block_len   :   %u\n",  dec_block_len);
    LOGI("enc_block_len   :   %u\n",  enc_block_len);
    LOGI("enc_msg_len     :   %u\n",  enc_msg_len);
#endif
    return enc_msg_len;
}
...
float fec_get_rate(fec_scheme _scheme)
{
    ...
    case LIQUID_FEC_CONV_V29P67:
    case LIQUID_FEC_CONV_V29P78:
        LOGE("error: fec_get_rate(), convolutional codes unavailable (install libfec)\n");
        exit(-1);

    case LIQUID_FEC_RS_M8:
        LOGE("error: fec_get_rate(), Reed-Solomon codes unavailable (install libfec)\n");
        exit(-1);
#endif

    default:
        LOGI("error: fec_get_rate(), unknown/unsupported scheme: %d\n", _scheme);
        exit(-1);
    }
    return 0;
}
...
fec fec_create(fec_scheme _scheme, void *_opts)
{
    ...
    case LIQUID_FEC_UNKNOWN:
        LOGI("error: fec_create(), cannot create fec object of type \"UNKNOWN\"\n");
        exit(-1);
    ...
    case LIQUID_FEC_CONV_V29P67:
    case LIQUID_FEC_CONV_V29P78:
        LOGE("error: fec_create(), convolutional codes unavailable (install libfec)\n");
        exit(-1);

    case LIQUID_FEC_RS_M8:
        LOGE("error: fec_create(), Reed-Solomon codes unavailable (install libfec)\n");
        exit(-1);
#endif

    default:
        LOGI("error: fec_create(), unknown/unsupported scheme: %d\n", _scheme);
        exit(-1);
    }
