...
void bpacketsync_print(bpacketsync _q)
{
    LOGI("bpacketsync:\n");
    LOGI("    p/n poly    :   0x%.4x\n", _q->g);
    LOGI("    p/n len     :   %u bytes\n", _q->pnsequence_len);
    LOGI("    header len  :   %u bytes\n", _q->header_len);
    LOGI("    payload len :   %u bytes\n", _q->dec_msg_len);
    LOGI("    crc         :   %s\n", crc_scheme_str[_q->crc][1]);
    LOGI("    fec (inner) :   %s\n", fec_scheme_str[_q->fec0][1]);
    LOGI("    fec (outer) :   %s\n", fec_scheme_str[_q->fec1][1]);
    LOGI("    packet len  :   %u bytes\n", _q->packet_len);
                             ...
                             unsigned int _bps)
{
    // validate input
    if (_bps > 8) {
        LOGE("error: bpacketsync_execute_sym(), bits per symbol must be in [0,8]\n");
        exit(1);
    }
                             ...
                             unsigned char _bit)
{
    ...
    case BPACKETSYNC_STATE_RXPAYLOAD:
        ...
        break;
    default:
        LOGE("error: bpacketsync_execute(), invalid state\n");
        exit(1);
    }
}
                                ...
                                unsigned char _bit)
{
    ...
    if ( fabsf(r) > 0.8f ) {
        ...
        LOGI("p/n sequence found!, rxy = %8.4f\n", r);
#endif
...
void bpacketsync_decode_header(bpacketsync _q)
{
    ...
    if (version != BPACKET_VERSION)
        LOGE("warning: bpacketsync, version mismatch!\n");
